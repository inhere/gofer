...
# Prefer GOFER_TOKEN (the default token_env): export it and let the server
# read it from the environment. Only fall back to --token TOKEN when GOFER_TOKEN
# is unset but a TOKEN was provided.
if [[ -z "${GOFER_TOKEN:-}" && -n "${TOKEN:-}" ]]; then
  ARGS+=(--token "$TOKEN")
fi
if [[ -n "$CONFIG" ]]; then
  ARGS+=(--config "$CONFIG")
...
if [[ -x ./dist/gofer ]]; then
  exec ./dist/gofer "${ARGS[@]}"
fi
