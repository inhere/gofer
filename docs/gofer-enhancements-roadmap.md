...
| v1.21 | 2026-06-28 | claude | **多 agent 协作合并 design + plan 已出**（design [`design/2026-06-28-multi-agent-collab-design.md`](design/2026-06-28-multi-agent-collab-design.md) v0.2 含对抗复审 7 处收紧；plan [`plans/2026-06-28-multi-agent-collab-plan.md`](plans/2026-06-28-multi-agent-collab-plan.md) 总纲 + 子 plan `multi-agent-collab/{P1-presence-mailbox,P2-roles,P3-supervisor-answerer}`）：四层模型（L1 通道 E28✅ / L2 身份信箱 E36 / L3 角色 E35 / L4 监督应答 E25），依赖 E33。4 决策：**并存·共抽象** · 覆盖 **E36+E35+E25 轻量安全** · **分层 answerer**（白名单自动答低危→升级人，E8≈拒答 confirmation）· **config `roles:` 段 + system_inject 模板**。复审收紧：agent_id 与 jobs.session_id 解耦 / agent_token 软隔离 / 自动答收窄 choice+options / **顺带修既有缺口**（job 终态对账残留 pending interaction，`InteractionCancelled` 现从未赋值）/ resume 重施 role / messages fan-out+TTL。新增面：2 additive 表 + 5 mcp 工具 + 6 `/v1` 端点 + roles 段 + 分层 answerer。bd epic `hyy-ai-inspect-hyxz`(P1=y2jg/P2=fl46/P3=axma)。**只设计未实施**，可进 SUPMODE。 |
...
| AUTO-02 | **定时任务（内置 cron）** | 中 | 中 | `schedules` 表 + 复用现有 sweeper loop 范式定时提交 job/工作流。问题：错过补偿、多 hub 重复触发（单 hub 不问题）。**触发位置（设计维度）**：① server 集中触发（默认、简单，复用 sweeper，worker 只执行被派的 job）vs ② 下发 worker 自触发（worker 内置调度器 + 本地 schedule，断网离线自治，但管理分散）——倾向默认①、②作可选高级。接 AUTO-03（定时+重试）、WF-03（定时跑导入的工作流）。属**自主化 epic**。 |
...
| MCP-01 | **监督 agent 自动应答** | 高 | 大 | `pending_interaction` 由另一"监督 agent"自动作答（用一个 job 答另一个 job 的提问）。⚠️ 失控/套娃/烧 token：定位**半自动**——自动答低危澄清，遇审批门(AUTO-01)/高危/超轮次**升级人**（经 OBS-07 IM 或 Web）。gofer 最有特色的"agent 编排 agent"。属**自主化 epic**。 |
...
2. ~~**MCP-02 `gofer mcp --server` client 模式 MVP**~~ —— **✅ 已落地**（2026-06-27，SUPMODE P1-P5，commit `b52901b`..`a21df1f`）：Backend 接口双实现 + 3 client 方法 + 模式分支（env 默认 client + `--standalone` 逃生），双 client 真机协作验收 PASS。**多 agent 协作 epic 下一片 = MCP-04 信箱原语**（agent 注册 + inbox 主动推送 + 双向寻址）。
...
6. **多 agent 协作 epic**（MCP-02 通道 + MCP-04 身份/寻址 + MCP-03 角色 + MCP-01 自动应答）：首片 **MCP-02 `--server` MVP 已在「便宜先行」item 2**；全 epic 需先出**合并 design**（见上「多 agent 协作 epic」横切），与自主化 epic 在 MCP-01 交叠、共享 AUTO-01/OBS-06/OBS-02。
