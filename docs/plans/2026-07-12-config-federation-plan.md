...
| P5 | `070caab` | NewJob/NewSchedule 加 worker-capability 级联：`selectedWorker`+`workerAgentKeys`+`projectOptions` 按所选 worker 的 caps 收窄 project/agent 下拉，`reconvergeToWorker` 换 worker/runner 重收敛，interactive 过滤(仅 NewJob)。**全 fail-safe**（空交集/无 caps 回落全量，绝不锁死）。主机 `pnpm build`(vue-tsc+vite) 双绿。**真 Chrome 级联冒烟 A/B/C/D/E 全 PASS**。 |
| P5-fu | `7b063d2` | **follow-up（方案 a，用户拍板）**：worker-only project 接进创建表单。`/v1/meta.projects` 并入在线 worker 上报 project（`worker_only:true`+空 allowlist，host 同名胜出，离线不并）；NewJob/NewSchedule 选中 worker 后纳入其 worker-only project、baseline/local 隐藏、切回丢弃重收敛；NewWorkflow 一行 `filter(!worker_only)` 排除（逐字不变）。契约单测 + 全量 Go 绿 + 主机 pnpm build 双绿 + **浏览器复验 5/5 PASS**（wonly 选 w1 后现身、baseline/NewWorkflow 无）。剩 tools-2gk 另两项后续。 |
