...
- [x] **P2-a** `NotificationConfig`（config）+ `event_deliveries` 表 + DAO（抢占领取/更新）
- [x] **P2-b** 投递入队（事件落库后按订阅 webhook 入 pending）+ webhook 客户端（HMAC/超时/白名单 `ValidateWebhookURL`）
- [x] **P2-c** 投递 sweeper（serve.go 挂载，退避表）+ 优雅停机
- [x] **P2-d** 投递可见性 `GET /v1/jobs/{id}/deliveries` + Web 状态 + example/config 注释
...
- **P2**（2026-06-20，commit `90b26c5`+`6253954`+`901affc`+`00cf0eb`）：`NotificationConfig`(webhooks{url/events/secret_env/projects}+allow_hosts+allow_http+max_attempts)+`ProjectConfig.NotifyEnabled`；`event_deliveries` 表 + DAO，**`ClaimDueDeliveries` 条件租约 UPDATE**(`WHERE id=? AND status=pending AND next_retry_at<=now` 单领取屏障 + 租约移出 due 窗口,at-least-once,SR303)；新 `internal/notify` 包：`ValidateWebhookURL`(https-only 除非 allow_http + **host 白名单 fail-closed** + `isPublicIP` 拒 loopback/private/unspecified/link-local/multicast,SR904)+`PostWebhook`(HMAC-SHA256 签名头 `X-Gofer-Signature` + ctx 超时 + 不跟 3xx)；`recordEvent` 落库后按订阅入队(项目 gate+默认集 job.terminal/interaction.created,best-effort)；投递 sweeper `startDeliveryLoop`(仿 startPruneLoop,15s ticker,**仅有 webhook 配置才启**,退避 30s→2m→5m→15m→60m SR606,超 max_attempts→failed)；`GET /v1/jobs/{id}/deliveries` + Web 投递面板 + example notification 段(防漂移测试)。验收门 5/5 PASS。**主控独立压测**:投递/抢占/退避测试 `-count=15` + `-race` 全绿(无 flaky 无竞争)；SSRF fail-closed + 11 类拒绝用例。决策:`postFn` 注入(测试驱动 claim→post→mark,生产走完整校验+签名);MQ 按 D6 排除。
