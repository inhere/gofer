...
```
- [x] `pnpm typecheck` 绿（T1~T7、T9 前端全部）。※ `pnpm build` 脚本本身即 `vue-tsc --noEmit && vite build`，build 绿即覆盖 typecheck。
- [x] `pnpm build` 绿（`/plans`、`/plans/:id` 懒加载路径存在，即 Plans.vue/PlanDetail.vue 已建）。主机侧由主控经 `gofer job -a exec` 独立复跑一次，非仅采信实施方输出。

**11.2 后端（容器内）**
- [x] `go build ./... && go vet ./...` 绿。
- [x] `go test ./internal/job/... ./internal/httpapi/...` 绿（T8 resume 继承 + T10 list counts）。
- [x] 全量 `go test ./...` 绿（G023 覆盖不降）。33 包 ok，`-p 1` 规避 runner/local 的 text-file-busy。

> ⚠️ **本计划测试清单的一处疏漏（已修，留痕）**：11.2 原只列 `internal/job` + `internal/httpapi` 两个包，**漏了 `internal/client` 的契约测试**。T10 让 list 内联 counts，改变了 `GET /v1/plans` 的响应契约，导致既有 `TestPlanClientRoundTrip`（断言 list 的 `Plan.Counts == nil`）失败。**只有跑全量 `go test ./...` 才会暴露**。
> 修法：升级断言而非删除 —— list 现在必须带 counts，同时保留"list 不带 jobs/todos"这一真正的轻量不变量。
> 教训：**改动 HTTP 响应形状时，受影响的不止 handler 包**；`internal/client` 有独立的 round-trip 契约测试。后续凡改响应结构，测试清单必须显式含 `internal/client`。
> 兼容性已核：全仓无 `DisallowUnknownFields`，`client.Plan` 对新增字段宽松解析，CLI `gofer plan list` 不受影响。
...
**11.4 收尾**
- [x] 更新总纲 `plan-orchestration-plan.md` 的 P4 checkbox（`- [ ] P4 …` → `- [x]`）+ 分子阶段提交（SR1202）。实际分 4 个 commit：T8+T10 后端 / T1-T6 前端 / T7+T9 前端 / client 契约测试回归修复。
- [x] 若 T10 落地，主纲「P4 outline / 分期表」补一句「list 内联 counts」。（v0.3 已在总纲 P4 行写入）
- [x] **用户眼检通过**（2026-07-10）：主机已重新部署 server，浏览器验收 11.3 运行期冒烟，OK。**P4 收官。**
...
- **D7 会话链回看入口（v0.2 新增，已定：做）**：9.3 的「继续会话」跳转新 job 后无回看整链入口；已归 plan 的 job 尚可从 PlanDetail jobs 表看到兄弟，**未归 plan 的独立 job 彻底断线**（而这正是 resume 主力场景）。数据层关联本就存在（同 `session_id`，`?session=` 后端三路已就绪），仅前端未透传。→ **T9.6**，纯前端三处小改（`ListJobsOpts.session` + `listJobs` 透传 + JobDetail 展开列表），无后端改动。
  - **未纳入本期**：无 `session_id` 的 job（exec 类一次性命令）的「快速重建」按钮 —— 它没有会话可续，需新增血缘字段 `source_job_id` + 服务端 `POST /v1/jobs/{id}/rebuild` 端点（env 值不出服务端，走 `env_set`/`env_unset` 变更集）+ `GET /v1/jobs/{id}/request` 翻默认为脱敏，跨 jobstore/job/httpapi/client/CLI/web 约 16 文件。**已划归 P5**（见 `P5-lineage-rebuild-plan.md`），不入 P4。
  - ⚠️ **与 P5 的 merge 冲突点**：T8 在 `resume.go:74-95` 的 `s.Submit(JobRequest{...})` 加 `PlanID: src.PlanID`；P5 在**同一处**加 `SourceJobID: jobID`。二者须并存，先落者勿覆盖后落者。
