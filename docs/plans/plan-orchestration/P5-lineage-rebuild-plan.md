...
> 上游：[P1-data-plan.md](./P1-data-plan.md)（✅ push）、[P2-mcp-aggregate-plan.md](./P2-mcp-aggregate-plan.md)（✅ push）、[P3-todo-plan.md](./P3-todo-plan.md)（✅ push）、[P4-frontend-plan.md](./P4-frontend-plan.md)（定稿待执行）
> 关联安全 issue：`h-aii-xqe1`（`GET /v1/jobs/{id}/request` 裸吐 request_json 含 env 明文）——本阶段 T7/T9 默认脱敏后**关闭**。
...
4. **resume 盖血缘**（T4）：`resume.go` Submit 加 `SourceJobID: jobID`（**与 P4 T8 的 `PlanID: src.PlanID` 同一处 merge，须并存**）。
...
7. **脱敏读视图**（T7）：`internal/job` 的 `RedactedRequest(id)`——env 值 wholesale→占位、prompt/cmd/cwd/system_prompt 走 `RedactString`，清 request_id/caller_id/session_id。供 `GET /request` **默认**返回。
...
- **env 明文永不出服务端**：源 env 真值只在服务端（request_json 落库 + `RebuildJob` 内存合并）；任何 HTTP 响应体（`GET /request` 脱敏、`GET /jobs` 的 `JobResult` 无 Env 字段、rebuild 返回新 `JobResult`）**都不含 env 明文**。
- **默认端点脱敏、无裸吐**：`GET /request` 默认脱敏（`h-aii-xqe1` 关闭）；裸吐分支删除——`rerun` 服务端化后无消费者（T9 已核验，仅内部 `execute.go`/`serve.go`/pty 直接读 `RequestJSON` 字段，**不经此端点**，不受影响）。
...
| `resume.go` Submit 尾部 `EscalateTo: src.EscalateTo,` + `})`，**当前无 PlanID 也无 SourceJobID** | `job/resume.go:74-95` | T4 加 `SourceJobID: jobID`；**P4 T8 会在同处加 `PlanID: src.PlanID`** → 谁后落谁合并（R1） |
...
| jobs UpsertJob 列/占位符/实参三处须同步；当前 34 列（plan_id 末列） | `jobs.go:172-227`（VALUES 34 `?` `:178`） | 加 `source_job_id`→**35 列**，三处同步 +1 |
...
(h) `UpsertJob` 三处同步 +1（34→35）：列清单（`:177`）末尾加 `, source_job_id`；VALUES（`:178`）**加一个 `?`**；ON CONFLICT（`:212`）末尾加 `,\n    source_job_id=excluded.source_job_id`；Exec 实参（`:226`）`rec.PlanID,` 后加 `rec.SourceJobID,`。
...
> ⚠️ 占位符铁律（P1 踩过）：列数=VALUES `?`数=Exec 实参数=35，改完数一遍。命名 `source_job_id`/`SourceJob`/`?source_job=`，勿与既有 `source` 列（执行位置）撞（R9）。

**验收**：`go test ./internal/jobstore/...` 绿；扩展单测：Upsert 两 job（`SourceJobID="j-src"`/`""`）→ `ListJobs{SourceJob:"j-src"}` 只回前者；`GetJob` 回读正确；旧库 Open→migrate 自动 ALTER + 建索引，旧行读 `""`。
...
### T4 — job 层 resume：续投 job 盖 `SourceJobID`（⚠️ 与 P4 T8 合并点）

**`internal/job/resume.go`**，`ResumeJob` 的 `s.Submit(JobRequest{...})`（现状 `resume.go:74-95`，尾部 `EscalateTo: src.EscalateTo,` `:94` 后、`})` `:95` 前追加）：
```go
		OriginAgent: src.OriginAgent,
		EscalateTo:  src.EscalateTo,
		// 血缘（P5）：续投 job 指回源 job。resume 语义 = source_job_id=源 id 且 SessionID 与源
		// 相同（上面 :84 已带 SessionID=src.SessionID）——据此区分"续会话"（rebuild 则 session 空/新）。
		SourceJobID: jobID,
	})
```

> **⚠️ 合并冲突点（R1）**：P4 T8（定稿待执行）在**同处**加 `PlanID: src.PlanID,`。后落者须两行并存：
> ```go
> 		OriginAgent: src.OriginAgent,
> 		EscalateTo:  src.EscalateTo,
> 		PlanID:      src.PlanID,   // P4 T8：续投继承源 plan
> 		SourceJobID: jobID,        // P5 T4：续投记源血缘
> 	})
> ```
> 二者正交无耦合，仅文本相邻。实施前看 resume.go 现状确认 P4 是否已落。
...
// SHOWS keys, values never leave the server), and prompt/cmd/cwd/system_prompt run
// through secret.RedactString (workflow parity). It also CLEARS read-noise: RequestID,
...
**验收**：`go test ./internal/job/...` 绿；单测：源 job 带 `Env{"API_TOKEN":"xyz"}`+`RequestID`+`SessionID` → env 值为 `***REDACTED***`、request_id/session_id 空、`redacted==true`；无 env/secret → `redacted==false`；未知 id → ok=false。
...
**验收**：`go test ./internal/job/...` 绿；单测：源 job 带 `Env{A:1,B:2}` → `RebuildJob(id, {EnvSet:{A:9},EnvUnset:["B"]})` 的新 job env=`{A:9}`、`SourceJobID==id`、`RequestID`/`SessionID` 空、`CallerID`=传入；空 overrides → 新 job 与源同请求（仅清 id/session、盖 source/caller）；override 含 `***REDACTED***` → `ErrRedactedPlaceholder`；未知 id → `ErrUnknownJob`。
...
(a) `Job`（现状 `:53-55`；P4 T1 会加 `plan_id`——勿重复）：
```ts
  escalate_to?: string
  plan_id?: string          // 归组键（P1；P4 T1 亦加，二者取一）
  source_job_id?: string    // 血缘键（P5）：本 job resume/rebuild 自哪个源 job
}
```

(b) `ListJobsOpts`（现状 `:412-423`；P4 加 `plan`/`session`——勿重复）：
```ts
  caller?: string
  plan?: string
  session?: string
  source_job?: string       // 血缘反查（P5）：列出某 job 派生出的所有 job
  limit?: number
  offset?: number
}
```
...
function buildRebuildBody(): RebuildBody {
  ...
  chg('timeout', timeoutSec.value, (v) => (b.timeout_sec = v ?? 0))
...
```
> `plan` meta 行由 P4 T9.4 加；P4 未落时本期可只加「派生自」行（`/plans/:id` 路由 P4 才有）。`RouterLink` 本模板已用（`:869`）。
...
- resume 一个 session job（若 P4 已落 UI）→ 新 job `source_job_id`=源、`session_id`=源（区分 resume vs rebuild）。
...
| 层 | 文件 | 用例要点 |
|---|---|---|
| jobstore | `internal/jobstore/jobs_test.go`（扩展） | source_job_id 加列回读；`ListJobs{SourceJob}` 过滤；旧库 migrate ALTER+索引，旧行读 `""` |
...
| job（脱敏） | `internal/job/*_test.go`（新） | `RedactedRequest`：env 值→占位、request_id/session_id 清、redacted 位；无 secret→false；未知 id→ok=false |
| job（rebuild） | `internal/job/rebuild_test.go`（新） | 空 overrides≡原样重投（仅清 id/session、盖 source/caller）；`env_set`/`env_unset` merge；未提 env key 保留原值；占位符 override→`ErrRedactedPlaceholder`；未知源→`ErrUnknownJob` |
...
**核心不变量**
- **env 明文永不出现在任何 HTTP 响应体中**：`GET /request` 脱敏（env 值占位）；`JobResult`（get/list/rebuild 返回）无 Env 字段；源 env 真值只在服务端（request_json + `RebuildJob` 内存合并）。
...
- **SQL 占位符三处同步**（指 `?`，与上一条无关）：`UpsertJob` 的 列名数 = `VALUES` 里 `?` 数 = `Exec` 实参数 = **35**（现状实测 34/34/34，加 `source_job_id` 后为 35；`jobstore/jobs.go` 的 `INSERT INTO jobs` 三处必须同增，漏一处静默错列）。
- **retry 保血缘**：`maybeRetryJob` 从快照恢复 `source_job_id`（因 `json:"-"` 不在 request_json）。

## 风险

- **R1 resume.go 合并冲突**：T4 的 `SourceJobID: jobID` 与 P4 T8 的 `PlanID: src.PlanID` 同处，后落者须并存。
...
- **D2 `env_set:{K:""}` 语义（已定）**：设为空串（**非**删除）。删除只走 `env_unset`，语义不重叠（`applyOverrides`：set 循环直接赋值、unset 循环 delete）。与用户倾向一致。
