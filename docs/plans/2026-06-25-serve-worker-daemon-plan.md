...
### 验收（P3）
- [ ] `gofer worker -d`（容器内，缺省读 `<config-dir>/worker.yaml`）后台起：打印 pid；`run/worker-w-docker-claude.pid` 存在；`run/worker-w-docker-claude.log` 有 "worker starting"；worker 在 host hub 上线（`gofer` host 侧 `/v1/runners` 可见）。
- [ ] `kill -TERM` / `gofer stop worker w-docker-claude`：going-away 优雅断连、pidfile 清理。
