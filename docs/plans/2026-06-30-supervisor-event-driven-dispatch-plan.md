...
- [ ] C1 数据层：`needs_human` 列 + `CountSupPendingDemand` + `MarkInteractionNeedsHuman` + 单测
- [ ] C2 reconciler：demand 闸 + wake 三路 select + 纯函数单测
- [ ] C3 router push：`Service.wake` 注入 + escalate 触发 + serve channel 连接 + 单测
- [ ] C4 ack-on-answer：answer 关联标 read + `gofer_punt_interaction` + sup prompt 改造
