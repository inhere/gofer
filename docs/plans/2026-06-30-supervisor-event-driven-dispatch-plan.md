...
- 不新增破坏性字段；live config（`D:/configs/gofer/config.yaml`）保持 `desired_supervisors=1` 即自动获得新行为。
...
- **E2E（容器 ephemeral serve，不碰 live）**：复刻 runbook §10 起一个临时 serve（独立 `storage.db_path` 便于清理），脚本验上面 5 条验收。**HOST 真机**（`/d/env/gopath/bin/gofer.exe`）经 `gofer job run -p workspace -a exec --runner local --sync` 重跑空闲零成本 + 按需唤醒两条（Windows 路径 / path_view:host）。
...
- [x] C5 config 语义注释 + loader — `f547bab`
- [x] E2E 容器控制面全绿（7/7, `tmp/ed-e2e/`）：事件驱动启动 / idle→零派发 / demand→派发 / active闸≤1 / punt→停止重派 — HOST 真机 sup 应答闭环沿用 §10 实测（本次未改 agent 接入）
- [x] runbook §11 事件驱动派发 + 头部指引 + §1-9/§10 收敛标注
- [x] live serve 重启切换 + 生产冒烟全闭环实证（2026-06-30）：idle 31min 零派发 / escalation→3s 按需派发 claude-sup / claude-sup 用新 `gofer_punt_interaction` 判 question 无白名单→留人(needs_human=1 落库) / 回归 idle 无 churn。issue y5wt 已关闭。
