...
- **E2E（容器 ephemeral serve，不碰 live）**：复刻 runbook §10 起一个临时 serve（独立 `storage.db_path` 便于清理），脚本验上面 5 条验收。**HOST 真机**（`/d/env/gopath/bin/gofer.exe`）经 `gofer job run -p example-project -a exec --runner local --sync` 重跑空闲零成本 + 按需唤醒两条（Windows 路径 / path_view:host）。
