...
- [ ] 验收：upsert→get 往返；ListPresence 顺序；TouchPresence 刷 last_seen；Prune 删 offline。
- [ ] 验收：InsertMessages 批量；ListInbox 只回 unread；MarkRead 后不再回；PruneMessages 清 read/过期。
- [ ] `go test ./internal/jobstore/...` 绿 → commit `feat(presence): jobstore agent_presence+messages 两表+DAO`
...
- [ ] Register 新建 vs 续约（同 name+caller 复用 id）；token 不为空。
- [ ] Poll：token 错→ErrUnauthorizedAgent；ack=true 后再 poll 空；ack=false 不消费。
- [ ] Post：直投 agent_id 1 行；role: fan-out 多行（各 online 一行）；无在线→delivered=0。
- [ ] List：last_seen 超 ttl→offline；role/project 过滤。
- [ ] `go test ./internal/presence/...` 绿 → commit `feat(presence): Service register/poll/post/list + role fan-out + token 校验`
...
- [ ] register→presence 可见；poll token 错 403；post 直投 delivered=1；poll 取到。
- [ ] `go test ./internal/httpapi/...` 绿 → commit `feat(presence): httpapi 5 端点(register/presence/poll/messages/deregister)`
...
- [ ] 验收：`go test ./...` 全绿；`go vet`/`go list -deps ./internal/presence`(无环)。
- [ ] commit `feat(presence): client 5 方法 + Backend 扩 4 + mcp 4 工具 + core/httpapi 线缆`
...
- [ ] 单测：`mcpUseClient`/CLI flag 绑定（照现有）。`go test ./...` 绿。
- [ ] **部署**：`go build -o tmp/gofer-linux .`；容器 `gofer worker stop → cp → gofer worker -d`；host 用户自建。
- [ ] **E2E（多 agent 协作语义，照 E28 plan 手法，双 mcp client）**：
  1. serve 起（host）。
...
- [ ] 回填主纲进度 + commit `feat(presence): gofer agent CLI + serve prune sweeper + E2E`。bd `y2jg` close。
