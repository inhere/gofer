...
| v0.5 | 2026-07-15 | Claude | **三审后把 v0.4 状态机的同步契约写到伪代码级（codex F 并发正确性，报告 `tmp/p3-review-out/F-statemachine-result.md`）**。F 确认 v0.4 **方向对**（「比双队列正确」）、`pendingPush` 不漏广播 ✅、有限公平无 livelock ✅；但同步契约缺失 → 2 BLOCKER + 4 HIGH，全是标准并发模式没写死：<br>① **🔴 F-B1 fence 后切 session 的 TOCTOU** —— 单次 pre-apply fence 是【点】、apply 是【段】(~2s)，fence 通过后 apply 期间 `beginSession(gen2)` 若执行则 gen1 配置仍发布。→ 加**共同提交锁 `applyMu`**（锁序 `applyMu→st.mu`）让「fence+apply+提交」与「切 gen」互斥；`pending` 带 gen 标签 `pendingPolicy{gen,policy}`；提交持 `st.mu` 再验 gen。改 T5-C/D。<br>② **🔴 F-B2 唤醒协议未定义 → lost-wakeup** —— 只写「唤醒 executor」没定义机制，裸 signal / 非阻塞 send 在 executor 未 park 时丢通知、Policy 无限期卡 pending。→ 容量 1 合并 `policyWake` channel；producer 锁下发布状态、解锁后非阻塞 `wake`；executor `select{ctx,reloadCh,policyWake}` 醒后重读状态。改 T5-C。<br>③ **F-H1 提交契约不全** —— `floor=max(lastRev,pendingRev)`、取出再验、成功持锁验 gen 后 `lastRev=max`、失败不推进。改 T5-C/D。<br>④ **F-H2 `markPolicyPending(max)` 未定义原子** —— 并发裸 `if rev>cur` 丢更新、Rev 回退。→ 复用 `wc.mu` 原子 max；`MarkPolicyApplied`/snapshot 同锁；沿用 `UpdateCaps` 锁序。改 T4。<br>⑤ **F-H3 `lastPolicy` 字段矛盾** —— T5-B 说它在 `sessionState` 但 T5-C struct 没列。→ 加进 struct，`beginSession` **不清**（LKG 跨 gen 保留）；`activeCfg` 明确 = core 当前原子 `Snapshot()`、不建第二指针。改 T5-A/C。<br>⑥ **F-H4 缓存回报自相矛盾 + 旧重试覆盖** —— 先发 Applied 后写缓存（帧发出改不了 stale）；Rev=5 迟到重试覆盖 Rev=6。→ 顺序改「持久化→组装 degraded→发 Applied」；每次 apply 生成单调 `applySeq` token，重试切 manifest 前验 token。改 T5-F。<br>**收敛信号**：三审首次无「方向性错误」，全是实现契约。 |
| v0.6 | 2026-07-15 | Claude | **锁序死锁审查后固化不变量（codex G，报告 `tmp/p3-review-out/G-lockorder-result.md`）**。G 把 server/worker 两张锁获取图喂 DFS、**均无环**（`cycles=[]`）——设计锁序正确。仅剩「防实施者写反」的实现不变量（非设计错），本版固化：<br>① **G-H1 `cacheMu` 锁归属**：`cacheMu` 临界区**绝不**获取 `st.mu/applyMu/updateMu`；缓存函数只收不可变 `Policy` 值 + `seq`；`applySeq` 用 atomic、在释放 `applyMu` 前发布；retry 持 `cacheMu` 只读 atomic token 做文件操作。防 `st.mu↔cacheMu` 环。改 T5-F。<br>② **G-H2 `flushPush` defer 陷阱**：禁 `defer c.flushPush()`（LIFO 会让 flush 先于 `Unlock` → 广播回 `updateMu` 内、D-HIGH-5 复发）；固定模板「显式 `Lock`→`reloadLocked`→显式 `Unlock`→`flushPush`」、抽 helper。改 T1-D。<br>③ **G-M1 `updateMu` 内禁阻塞 I/O**：`agent.Resolve`(~3s 子进程) + `config.Save` 已在锁内（接受慢 apply），但**禁**再叠网络 / 阻塞 channel / sleep / 缓存 fsync；缓存 rename/fsync 只在所有锁释放后。改 T1-D。<br>④ 小：`runReload` 绝不自己再加 `applyMu`（否则 `tryApplyPending→runReload→applyMu` 自锁）；`offerPolicy` 的 `wake` 改显式 `Unlock` 后再调（与 `tryApplyPending` 契约一致）。<br>**✅ G 实跑 DFS 确认**：server `registry→wc.mu` / `Service→entry` 两单向 DAG（`UpdateCaps`/`All`/`Get` 返回前释放 registry 锁）；worker `applyMu→st.mu/updateMu/capsMu` 单向、无 `st.mu→applyMu` 反序、worker-core `reloadLocked` 不碰 `applyMu/st.mu`。**六轮审查（A–G）收敛完成**。 |
...
**修法（并入 v0.4 的单一 session 状态机；v0.5 补齐同步契约）**：Policy **不走**有界 `reloadCh` FIFO。`sessionState` 里带 `pending *pendingPolicy`（带 gen 标签、latest-wins：`floor=max(lastRev,pendingRev)`，`rev>floor` 才替换）；`offerPolicy(gen,p)`（recvLoop 的 `TypePolicy` / catch-up 都走它）替换后 `wake`。SIGHUP/远程 reload 仍走 `reloadCh`、与 Policy 由**同一** executor 串行、**有限公平**。完整并发契约（`applyMu` 共同提交锁 + 容量 1 `policyWake` + apply 前 fence）见 T5-C。（改 T5-C。）
...
21. **🔴 跨连接 Policy 不复活（B4，session generation）**：executor 里堵着一个旧会话的 `Policy{Rev:100}` 未执行 → 完成新会话握手（新 gen，Rev 从小数）→ 旧 Policy **不得**在新会话生效。
    ...
    - 🔴 **beginSession 必达（D-B1）**：`beginSession` 不入 `reloadCh`、直接改受锁 state + `wake` → 阻塞 executor + 填满 8 个 reload → 新握手 → 断言 gen 已切、新 Policy 最终应用。**先证伪**：让 `beginSession` 走可丢的 `enqueueReload` → 队列满必须复现「gen 不切、新 Policy 全丢、worker 永久停旧」。
    - 🔴 **fence 后切 session（F-B1）**：executor 取出 gen1 pending 后、apply 那 ~2s 内触发 `beginSession(gen2)` → 断言 gen1 配置**不发布到 core**（被 `applyMu` + 步骤 3/5 的 fence 拦下）、gen2 的 `lastRev` 不被 gen1 污染。**先证伪**：去掉 `applyMu`（只留单点 fence）→ 必须复现「gen2 已发布后 gen1 又提交」。
    - 🔴 **lost-wakeup（F-B2）**：executor 检查 `pending==nil`、正要 park 的窗口里 `offerPolicy` 设 pending + `wake` → 断言 Policy **最终被 apply**、不永久卡。**先证伪**：把 `policyWake` 换成裸 `Cond.Signal`（无 predicate 循环）或去掉容量 1 合并 → 必须复现「通知丢、Policy 卡 pending 到下次 reload」。
...
- 🔴 **禁 `defer c.flushPush()`（G-H2）**：`Update`/`Reload` 用 `defer c.updateMu.Unlock()`，若再 `defer c.flushPush()`，LIFO 会让 flush **先于** unlock 执行 → 广播回到 `updateMu` 内、D-HIGH-5 复发。**固定模板**：显式 `Lock` → `reloadLocked` → 显式 `Unlock` → `flushPush`（不用 defer 拼接）；建议抽一个只负责锁内事务的 helper，公共入口在 helper 返回后调 `flushPush`。测试：`PushPolicyAll` hook 里并发 `Update`，断言 hook 执行时另一 goroutine 能拿到 `updateMu`。
- 🔴 **`updateMu` 内禁阻塞 I/O（G-M1）**：锁内已有 `agent.Resolve`（~3s agent 子进程探测）+ `config.Save`（写盘）——**接受**（慢 apply 是已知代价）；但**禁止再叠**网络写 / 阻塞 channel 发送 / `sleep` / 缓存 fsync。Policy 缓存的 rename/fsync 只在锁释放后（见 T5-F）。
...
- 🔴 **`markPolicyPending(max)` / `MarkPolicyApplied` 的原子性（F-HIGH-2）**：三处下发（ack / catch-up / `PushPolicyAll`）可能并发同一 `wc`，裸 `if rev>cur { cur=rev }` 会丢更新、pending Rev 回退。**复用现有 `wc.mu`**：锁下 `if rev>wc.policyRev { wc.policyRev=rev; wc.policyPending=true }`；`MarkPolicyApplied` 与 `WorkerSnapshot` 读取同用这把 `wc.mu`。锁序沿用 `UpdateCaps`（先 registry `RLock` 确认 `r.conns[id]==wc`，再 `wc.mu`）。最好帧**成功写出后**才提交 pending（写失败连接即关），避免「未下发却 pending」的假阳性。
...
- `activeCfg = 当前实际运行的配置`（POLICY 下 = 上次投影结果；**从不被「没有 Policy」这件事清空**）。🔴 **不是 worker 上的第二个指针字段（F-HIGH-3）**：它 = **core 当前发布的原子 `Snapshot()`**（worker 侧 core，T1-A）；no-op 判断从这个一致性入口读，不另建可变指针（否则与配置发布撕裂）。`lastPolicy` 则是 `sessionState.lastPolicy`（T5-C，跨 gen 保留的 LKG）
...
```go
// internal/worker：sessionState 是 gen/pending/lastRev/lastPolicy 的唯一真源；三方经它会合
type pendingPolicy struct { gen uint64; policy wsproto.Policy }   // 不可变值，带 gen 标签（H1/B1）
type sessionState struct {
    mu         sync.Mutex
    gen        uint64          // 当前会话世代（每次成功握手 +1）
    lastRev    int64           // 本会话已应用最高 Rev（会话切换清零）
    pending    *pendingPolicy  // latest-wins：当前 gen 待应用的最新输入
    lastPolicy *wsproto.Policy // ★ H3：最近一次成功 apply 的完整快照（LKG，【跨 gen 保留】，beginSession 不清）
}
// 两把锁 + 一个容量 1 合并唤醒：
//   applyMu    —— ★ B1 共同提交锁：让「executor 的 fence+apply+提交」与「beginSession 切 gen」互斥。锁序恒 applyMu → st.mu
//   policyWake chan struct{}（cap=1）—— ★ B2 合并唤醒。wake = 非阻塞 select send（满即丢，token 已在）
func wake(ch chan struct{}) { select { case ch <- struct{}{}: default: } }

// 握手成功（runSession，连接 goroutine）——唯一入口，不可丢
func (cl *Client) beginSession(gen uint64, ackPolicy *wsproto.Policy) {
    cl.applyMu.Lock()                                        // ★ B1：与 executor apply 提交互斥；最多等一次在跑的 apply(~2s)
    cl.st.mu.Lock()
    cl.st.gen = gen; cl.st.lastRev = 0; cl.st.pending = nil  // 原子切 gen + 清 Rev + 丢旧 gen pending
    if ackPolicy != nil { cl.st.pending = &pendingPolicy{gen, *ackPolicy} }
    // ★ H3：lastPolicy 【不清】—— LKG 跨重连保留
    cl.st.mu.Unlock(); cl.applyMu.Unlock()
    wake(cl.policyWake)
}

// recvLoop 收 TypePolicy / catch-up 帧——只持 st.mu（不被 apply 阻塞）
func (cl *Client) offerPolicy(gen uint64, p wsproto.Policy) {
    cl.st.mu.Lock()
    floor := cl.st.lastRev                                   // ★ H1：接收下限 = max(lastRev, 已存 pending.Rev)
    if cl.st.pending != nil && cl.st.pending.gen == gen { floor = max(floor, cl.st.pending.policy.Rev) }
    ok := gen == cl.st.gen && p.Rev > floor
    if ok { cl.st.pending = &pendingPolicy{gen, p} }
    cl.st.mu.Unlock()                                        // ★ G：显式 unlock，wake 在锁外（与 tryApplyPending 契约一致）
    if ok { wake(cl.policyWake) }                            // 旧 gen / 旧 Rev → 不设不唤醒
}
```

executor（P1 既有 `reloadLoop`，**不新建 goroutine**，只加第三个唤醒源）：

```txt
select {
  <-ctx.Done()        → 退出
  req := <-reloadCh    → runReload(req)         // SIGHUP/远程，P1 既有；★有限公平：处理一个就回 select
  <-policyWake         → tryApplyPending()
}

tryApplyPending():
  1. 持 st.mu：p:=pending；若 p==nil || p.gen!=st.gen || p.policy.Rev<=st.lastRev → 解锁 return（H1 取出再验）
     记 localGen,localPol := p.gen,p.policy；pending=nil；解锁
  2. 🔴 拿 applyMu（B1；可能等 beginSession 释放）
  3. 🔴 fence：持 st.mu 读 gen，若 localGen!=st.gen → 释放两锁 return（「取 pending」与「拿 applyMu」之间 beginSession 切了 gen → 丢）
  4. runReload(&localPol)：projection+agent.Resolve+detect+ReloadWith+storeCaps（慢~2s，全程持 applyMu → beginSession 进不来）
  5. 🔴 提交：持 st.mu，若 localGen==st.gen → lastRev=max(lastRev,localPol.Rev)、lastPolicy=&localPol（H3 成功才更新 LKG）；解锁
  6. 释放 applyMu；writeApplied + writePolicyCache（T5-F）；wake(policyWake)（apply 期间若来更新 pending，自续）
```

- 🔴 **B1 共同提交锁封住「fence 后切 session」TOCTOU（F-BLOCKER-1）**：单次 pre-apply fence 是【点】，apply 是【段】(~2s)——F 证：fence 通过后、apply 期间 `beginSession(gen2)` 若能执行，gen1 配置仍会发布到 core（gen2 已发布后 gen1 又提交）。`applyMu` 让「fence+apply+提交」与「beginSession 切 gen」互斥（锁序 `applyMu→st.mu`），只剩两种合法顺序：旧 apply 提交完 gen2 才发布 / gen2 先发布则步骤 3 的 fence 丢弃旧 apply。代价：`beginSession` 最多等一次 apply(~2s)、握手非热路径、可接受。
- 🔴 **B2 合并唤醒防 lost-wakeup（F-BLOCKER-2）**：容量 1 的 `policyWake`；producer 先在 `st.mu` 下发布状态、解锁后 `wake`（非阻塞 send）；executor 阻塞点 `select{ctx, reloadCh, policyWake}`，收到 wake 只表示「可能有活」、**必须重读 `st.mu` 查真实状态**（channel 不承载 Policy）。容量 1 保证：executor 没 park 时 token 留 channel、apply 期间多次通知合并、清 pending 后的新 offer 能重新留 token。
- 🔴 **ack 内嵌 Policy 走同一入口（D-HIGH-3）**：握手只调 `beginSession(gen, reg.Policy)`；**禁止 `runSession` 直接写 `lastPolicy/lastRev` 或直接调 Reload**。独立 `TypePolicy` 走 `offerPolicy` —— 两条汇到同一个 `sessionState`。
- 🔴 **`runReload` 是唯一执行体** —— `cl.storeCaps(caps)` 是重连 register 帧 caps 的唯一来源（§4.3）。另写 apply 路径 = 重连用过期 caps 注册（静默）。
- 🔴 **有限公平（无 livelock，F 复核 ✅）**：executor 每轮 `select` 最多处理一个 reload 或一次 `tryApplyPending`，**不内层无限 drain pending**——持续 Policy 不饿死 reload、持续 reload 不饿死 Policy；`tryApplyPending` 取的是固定 `localPol` 快照，apply 目标不会中途漂移。
- 🔴 **锁序不变量（G 实跑 DFS 无环 ✅）**：worker 侧锁序**恒** `applyMu → st.mu`、`applyMu → (worker-core)updateMu → availabilityRecorder.mu`、`applyMu → capsMu`——**无反向**。写死：`runReload` **绝不自己再加 `applyMu`**（否则 `tryApplyPending→runReload→applyMu` 自锁）；`cacheMu` 临界区**绝不**取 `st.mu/applyMu/updateMu`（G-H1，见 T5-F）。
...
- 会话内：`offerPolicy` 用 `floor=max(lastRev, pendingRev)` 收 pending（H1）；executor **步骤 3 在 `applyMu` 下 fence、步骤 5 提交再验 gen**（不是单点 fence，B1）。**旧 gen 先按 gen 丢弃**，轮不到比 Rev。
- 🔴 **`lastRev` 会话切换清零、`lastPolicy` 跨 gen 保留（H1/H3）**：`lastRev` 是「本会话」的 Rev 高水位（`beginSession` 清 0）；`lastPolicy` 是「最近一次成功 apply 的完整快照」= LKG，`beginSession` **不清**，供冷启动/降级恢复（T5-A/F）。两者语义不同，别混用一个字段。
...
seq := nextApplySeq()                           // ★ H4：本地单调 token，绑定这次成功 apply
cacheErr := writePolicyCache(effPolicy, seq)    // ★ 先持久化（temp+rename 切 manifest，带 seq）——在发 Applied 之前
degraded := diagnose(cfg, effPolicy, wc)        // 【只读】：policy 允许但本机没装的 agent / 被 guards 收紧的 exec/interactive
if cacheErr != nil { degraded = append(degraded, {"*","policy_cache_stale"}); 使旧槽失效; 排入带 seq 的重试 }
caps := workerCaps(wc, cfg, det.snapshot())     // ★ POLICY 模式下 Projects 改从 cfg.Projects 取
writeApplied(Applied{Rev, &caps, rejected, degraded})   // ★ 最后才发 Applied（degraded 此刻已定）
```
...
- 🔴 **缓存回报顺序 + 旧重试防覆盖（D-HIGH-6 + F-HIGH-4）**：
  - **顺序**：apply 成功 → 尝试持久化/失效旧槽 → 据结果组装 `degraded` → **最后**才 `writeApplied`。v0.4 曾先发 Applied 后写缓存，帧发出就改不了 `policy_cache_stale`（F 坐实的自相矛盾）。
  - **旧重试防覆盖**：每次成功 apply 生成本地单调 `applySeq` token；重试在切 manifest 前必须在 `cacheMu` 下验 `token == 当前 LKG token`，不等即丢弃（否则 Rev=5 的迟到重试会把 manifest 切回 5、覆盖已写的 Rev=6）。**不用 Policy Rev 作跨 session 的缓存新旧判据**（新 server 的 Rev=1 可合法替换旧 Rev=100）。
  - **使旧缓存失效**：写失败时让旧槽失效，避免重启把**过期 Rev** 当 LKG 恢复、重放旧权限（例：apply Rev=41 撤了权限但缓存卡在 40，重启 + server 不可达就把旧权限放回来）。
  - **双槽 + 小 manifest**：先完整写/校验新槽，apply 成功后**受同一 token 校验**的原子切 manifest（rename 只防半文件、不防旧写者晚到）；冷启动只恢复 manifest 指向且校验通过的版本。
  - 🔴 **`cacheMu` 锁归属（G-H1，防 `st.mu↔cacheMu` 环）**：`cacheMu` 临界区**绝不**获取 `st.mu/applyMu/updateMu`；`writePolicyCache`/retry **只收不可变 `Policy` 值 + `seq` 参数**（不读 `st`）；`applySeq` / 当前 LKG token 用 **atomic**、在**释放 `applyMu` 前**发布，retry 持 `cacheMu` 后只读 atomic token 做文件/manifest 操作。缓存的 rename/fsync **只在 `applyMu/st.mu/updateMu` 全部释放后**做（G-M1）。
...
| 🔴 **F-B1 fence 后切 session TOCTOU → gen2 已发布后 gen1 又提交** | **T5-C** `applyMu` 共同提交锁（fence+apply+提交 与 切 gen 互斥，锁序 applyMu→st.mu）+ pending 带 gen 标签；**验收 21 fence-后-切换先证伪** |
| 🔴 **F-B2 唤醒未定义 → lost-wakeup、Policy 卡 pending** | **T5-C** 容量 1 合并 `policyWake`（producer 锁下发布 + 解锁后 wake，executor 醒后重读）；**验收 21 lost-wakeup 先证伪** |
| **F-H1/H2/H3/H4 提交契约 / pending 原子 / lastPolicy 字段 / 缓存顺序** | **T5-C/D · T4 · T5-A/F** `floor=max`；`wc.mu` 原子 max；`lastPolicy` 进 sessionState 不清；缓存先持久化后发 Applied + `applySeq` token |
| **G-H1/H2/M1 实现期锁环 / 锁内阻塞**（G 实跑 DFS **无环**，剩防写反） | **T5-C/F** `cacheMu` 不取 `st.mu/applyMu/updateMu`、`runReload` 不重入 `applyMu`；**T1-D** 禁 `defer flushPush` + `updateMu` 内禁阻塞 I/O |
...
- [ ] **T4** hub：ack 带 Policy（Q7-b）+ **catch-up push（N1）** + 锁外 `flushPush` 广播 + `TypeApplied` → `reg.UpdateCaps` + **policy_pending 只换文案（H1）+ 置位/清除按 Rev 单调、`markPolicyPending(max)` 用 `wc.mu` 原子（E-HIGH-1 / F-H2）**（fake worker 验帧序）— **验收 7/15 先证伪**
- [ ] **T5** worker：三态判定（E-B2）+ 完整快照替换不 merge（E-B1）+ LKG 恢复 / **单一 session 状态机（`beginSession`/`offerPolicy` + `applyMu` 共同提交锁 + 容量1 `policyWake` + pending 带 gen + `lastPolicy` 进 struct 不清 + 提交 `floor`/验 gen；F-B1/B2/H1/H3）** / P1 串行 executor（复用 `runReload`、有限公平）/ roots 映射 / 白名单不交集 / H2 两字段 / **缓存先持久化后发 Applied + `applySeq` token（F-H4）** — **验收 1/2/8/9/13/14/16/21/22/26 先证伪**
