...
- [x] **P2-a** `client.ListJobs(opts)` + `client.StreamJob(ctx,id,from,onEvent)`（SSE 帧解析助手）+ `client.GetJobRequest(id)`
- [x] **P2-b** `GET /v1/jobs/{id}/request` 端点（回原始 JobRequest）+ 路由
- [x] **P2-c** `job list`/`watch <id>`/`rerun <id>` 子命令 + `gofer completion` 接线
- [x] **P2-d** tags 设置入口（`job run --tags` + md+yaml frontmatter tags 核对 + Web NewJob tags 输入）
- [x] **P3-a** `gofer init`（写 starter yaml，`--force` 守卫）+ `gofer config validate`（全局 load+validate+逐项目 reg.Validate）
- [x] **P3-b** `config/gofer.example.yaml` 补 callers/workers/runner_probe/retention/db_path/worker-runner 段
...
- **P2**（2026-06-20，commit `056c3da`+`9ba24e2`+`0787181`+`013c978`）：`client.ListJobs`/`GetJobRequest`/`StreamJob`——SSE 解析**上抽为共享 `client.ParseSSE`/`SSEEvent`**，`runner/peerhttp` 改用它并删本地 `sse.go`（比 D4 更进一步，单一解析器，peerhttp 回归绿）；client 复用 `job.ListOpts`/`JobResult`/`JobRequest`（依赖单向无环）。`GET /v1/jobs/{id}/request` 只读端点（`json.RawMessage` 直出，不改 get_job）。`job list`（表格）/`watch <id>`（status+log 实时，终态 exit code done=0/cancelled=130/other=1，Ctrl-C→130）/`rerun <id>`（GetJobRequest→**清 `RequestID`** D5→SubmitJobSync，`--watch` 可跟）。`gofer completion bash|zsh`（自建 stdout 输出，零新依赖；未用 builtin 交互式）。P2-d 设置入口：`job run --tags a,b`（仿 splitLabels）+ NewJob 表单 tags + md frontmatter tags（本就经 yaml.Unmarshal 支持，补测）。验收门 5/5 PASS，全量含 peerhttp/worker 回归绿。

- **P3**（2026-06-20，commit `b4e29f8`+`313d8c8`）：`config/gofer.example.yaml` 补 callers/workers/runner_probe/retention/db_path/worker-runner 段（每段带注释，字段逐字段对齐 model.go yaml tag）；新建 `config/embed.go`（包 `configtmpl`，`//go:embed gofer.example.yaml`——go:embed 不能跨父目录，故 example 原地作单一真源）。`gofer init [--config][--force]`（写 starter，已存在/写失败 coded error exit≠0，比计划更严）+ `gofer config validate`（`Load`+`validate`+逐项目 `reg.Validate` 复用 project 校验，project keys 排序稳定输出，任一 FAIL 非零退出）。验收门 5/5 PASS。**防漂移测试** `TestExampleYAMLParses`/`TestExampleCommentedBlocksDecode`（反注释可选段实解码，含 tag/字段不同名陷阱）。真机冒烟：init(0)/拒覆盖(2)/validate 坏配置(2)/job 子命令 wiring/completion 全 PASS。
