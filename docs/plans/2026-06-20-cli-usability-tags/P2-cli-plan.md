...
## P2-d tags **设置入口**（闭合 E5 端到端，SR1402）

> P1 接通了 tags 数据路与**检索**侧；但用户得能**设置** tag 才谈得上按 tag 检索。补设置入口：

### 落点
- `internal/commands/job.go` `job run`：加 `--tag`（gcli `Strings`，可重复 `--tag a --tag b`；或 `--tags a,b` 逗号分隔，**与现有 `--worker-labels` 的 `splitLabels` 风格一致**），填入 `JobRequest.Tags`。
- md+yaml 提交：确认 frontmatter 解析已能带 `tags`（`JobRequest.Tags` 有 yaml tag，P1 已加）——若 md 解析器走结构体反序列化则自动支持，**核对并补测**；不支持则在解析处补。
- `web/src/views/NewJob.vue`：提交表单加 tags 输入（逗号分隔或 chips），并入提交 payload；`api/types.ts` 的提交请求类型加 `tags?: string[]`。

### P2-d 验收
- 单测 `internal/commands`：`job run --tag a --tag b`（或 `--tags a,b`）→ 构造的 `JobRequest.Tags == ["a","b"]`。
- md+yaml：带 `tags: [x,y]` frontmatter 的提交 → job 落库 tags 正确（补一个解析测试）。
- `pnpm -C web build` 绿；NewJob 提交带 tags → 详情/list 可见、可被 `--tag` 过滤（真机冒烟接 P2-c 的 list）。

---

### 提交点
P2-a / P2-b / P2-c / P2-d 各绿灯分别 `git commit`；更新主纲进度 + 实施结果一行。
