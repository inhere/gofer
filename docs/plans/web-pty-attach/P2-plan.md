...
- [x] Follow-up(2026-07-06)：serve-local interactive attach 接入同一 `ptyrelay`/`pty_sessions`，补齐 P2 当时 defer 的 local 路径。
...
// SetObserver 由 worker 命令在 worker.Serve 前注入；serve-local attach 后续也可注入自己的 observer。
...
```go
	...
	if pr, ok := cr.Runners[ptyrunner.Name].(*ptyrunner.PtyRunner); ok {
		pr.SetObserver(cl) // worker: observer=Client；serve-local attach 使用 serve 自己的 observer
	}
