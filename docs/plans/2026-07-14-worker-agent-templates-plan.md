...
`builtinTemplates`：**完整定义**（type / command / args / interactive / no_raw_cmd / detect），覆盖 `claude` / `codex` / `opencode` / `tty-claude`（`exec` 已内置，**不要重复定义**）。

- **`tty-codex` 若无法确证其正确形态，本期先不放**（宁缺毋错；线索只有 `registry.go:99-116` 的 `resume {{session_id}}`）。
...
- **协议版本**：`*bool` 方案下**无需 bump**（`As[T]` 是裸 `json.Unmarshal`，无 `DisallowUnknownFields`）。若最终选择 bump 到 v4，**P3 的 Policy 必须顺延 v5**（M4）。
...
9. proto=2 旧 worker 仍能连、能干活（复用 P1 的 git worktree 编旧二进制手法）
```
...
- **`localAgentCaps()` 不套 detect 门控**（本期）：它是 worker caps 的孪生路径，套上去会让 `/v1/runners` 的 local 能力视图塌（前端级联丢 agent），且**不会**导致 job 被拒（local 准入不走 caps 门）。→ **本期显式决策：local 侧只做 materialize，不做 detect 剔除**；语义差异写进注释，避免两条路径无声漂移。
...
- [ ] **T0 地基**：写回隔离（A）+ merge 恰好一次（B）+ 装配路径收敛（C）+ Detector seam（D）
