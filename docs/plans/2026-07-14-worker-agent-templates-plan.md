...
- [x] T1 内置模板注册表（5 条；tty-codex 已放，待 T6 pty 实测）— `46c3851`。**模板里不放 `detect` 块**（T2 后 Available 由 LookPath 定，detect 只影响 version 串且缺省即 `<command> --version`，显式写=重复+漂移风险）。tty-codex 经 `builtinSessionDefaultFor` 的 command-基名 fallback **继承 codex 的 session 默认值**（已测）
- [x] T2 detect 重构（exec 免探前置 / LookPath 定 Available / 并行）— `b62dbd8`。实测：6 探针全 hang → **2.0029s** 返回（改造前串行最坏 30s）。**D13 的假阴性已消除**：装了但无 detect 块的 agent，`/v1/agents` 现在报 `available:true`。另定：`DetectResult.Error` 在 `Available=true` 时**恒空**（防消费方拿 Error 当「不可用」用）
- [x] T3 接进 register/reload 上报链 + `AgentBrief.Available *bool` — `3519a1e`。铁律 1 已证伪验证（临时加 detect 过滤 → 测试立刻红，正如 live 会坏）。**结构性保证**：agent 集由 `resolvedAgentKeys` 只读 `cfg.Agents` 算出，看不到 detected → availability 只能装饰、不可能过滤。消费侧逐一核查：无人拿 `Available` 做过滤
- [x] T4 护栏（`!remote` 块内、基于 `gateAgent`）+ **前端非交互下拉过滤** + resume 回归测试 — `5639f0b`（顺带修了计划漏掉的 `NewSchedule.vue`：调度 job 恒为非交互，列出的 tty-* 是 100% 失败项）
- [x] T5 探测缓存（`agent.Registry.Availability()`；覆盖 `/v1/agents` **与 MCP `ListAgents`**）。cfg+avail 收进**同一个** `atomic.Pointer[registrySnapshot]`，随 `ReloadWith` 一起换代（读者永远不会把新 cfg 配旧探测结果）。喂缓存的就是 `agent.Resolve` 的第二个返回值（T0 那唯一一次 detect）。
  - **防假阴性两条不变量**（缓存不得凭空"弄丢"一个装了的 agent）：① **冷缓存**（`NewRegistry`，即 `internal/commands` 的三条 core-less 路径与测试 fixture）→ **现探一次并记忆**，绝不读成"全部不可用"；② **exec 类型恒可用**（走 `lookPathProbe`，不看缓存）——`NoopDetector` 什么都不报，不能因此把内置 exec 探没。
  - **`gofer agent detect` 保持现探**（`Registry.Detect` 单探针未动）：它的语义就是"现在给我探一次"，一次性进程，无缓存可吃。
  - **新装的 CLI 何时被发现**：reload（SIGHUP / `gofer worker reload`）——每个新 cfg 快照跑一次 detect，结果连同 cfg 原子换进缓存。不做定时自刷新（availability 是展示事实，永不作准入门）。
- [x] T6 e2e 冒烟 — **8 条验收全 PASS（22 项断言，0 FAIL）**
  - 验收 1：旧(`755b0fd`)/新二进制对同一份现网形态 worker.yaml → `agent_caps` **diff 为空**；探测失败的 `tty-demo` 仍在 caps（铁律 1 真机成立）
  - 验收 2：`POST /v1/projects` 与 `gofer project add` 后，config.yaml **无 `agents:` 段、无任何模板 key**
  - 验收 7：真 proto=2 旧 worker（`4def378`）的 `available` 全部是 **null（unknown）而非 false** → `*bool` 的选择被真机坐实
  - 验收 8：reload 端到端 **93ms**（预算 10s）
  - 验收 10：server 删 agents 段 + 本机没装 claude → **只有交互 job 400**，非交互 job 照跑 → **必须进迁移文档**
  - 前端（真浏览器，贴 DOM）：非交互下拉 `["claude","exec"]`（无 tty-*）／交互下拉 `["tty-claude"]`／NewSchedule `["claude","exec"]`
  - **tty-codex 保留**：主机 ConPTY 实测跑通完整回合（TUI 启动→打字→Enter→模型响应）。已知行为（codex 自身，非 gofer bug）：TUI 启动 ~20-30s，此前输入不提交；未登录机器落到 sign-in 向导（远程 worker 选 Device Code）
