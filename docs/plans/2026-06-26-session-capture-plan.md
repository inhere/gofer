...
> 依据设计：[`design/2026-06-26-session-capture-design.md`](../design/2026-06-26-session-capture-design.md) v1.1（bd `example-project-nnk` / roadmap E33）。
...
- **验收**：`-a codex`（若容器装）/ 或 `-a claude --output-format json` 的 `--runner w-container-example` job → host `job show` 有 session_id。
