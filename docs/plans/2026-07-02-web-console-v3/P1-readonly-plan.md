...
- [x] **P1.7b** 后端 · running 时 persist `rendered_command` ✅（execute.go running 转换处锁内 set；build+全量 test 绿）— #5 running 即显真正兑现
...
- **前端构建在主机**（容器 node_modules 是 host 符号链接农场，不能直接 build）：`gofer job run -p example-project -a exec --runner local --cwd tools/gofer/web --timeout 600 --sync -- pnpm build`（= `vue-tsc --noEmit && vite build`）。
