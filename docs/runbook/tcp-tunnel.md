...
```yaml
tunnel:
  allow: ["192.168.1.10:502", "192.168.1.0/24:502"]   # 尽量精确到设备与端口，避免放开整网段任意端口
  max_conns: 8
  dial_timeout_sec: 5
```

条目支持 `host:port`、`CIDR:port`、任意端口 `*`，IPv6 使用 `[2001:db8::10]:502`。修改后执行 `gofer worker reload <worker-id>`，或重启 worker。worker 必须支持协议 v5。
