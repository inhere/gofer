...
```
host serve(进程 env)
  └─ gofer job run -a codex-sup            # 起一个 codex job(role 预设可选)
       └─ local runner: cmd.Env = os.Environ(serve) + agent.env(codex-sup)   # ← 见 §3
...
要点：env 只能 per-agent 配置。**绝不能**把 `GOFER_AGENT_ROLE` 设到普通 `codex` agent 或 serve 进程 env 上——那会让**每个** codex job 的 mcp 都注册成 supervisor，被 answer 闸误收口。正确做法：**定义一个 sup 专用 agent（如 `codex-sup`）只给它配 `env`**。

> 注意（坑）：内置 session/system 默认是**按 agent 名**兜底的（`builtinSessionDefaults["codex"]`，`registry.go:89`）。新名 `codex-sup` **不会**自动继承 codex 的 `system_inject`/`session_resume`，需在配置里**显式补**。
...
# --- sup 专用 agent：codex + 仅给它配 role env（不污染普通 codex job）---
agents:
  codex:                               # 普通 codex（无 GOFER_AGENT_ROLE → 注册 role 为空）
    type: cli-agent
    command: codex
    args: [exec, "{{prompt}}"]
    detect: { command: codex, args: [--version] }

  codex-sup:                           # sup 专用：唯一区别是 env 注入 supervisor 身份
    type: cli-agent
    command: codex
    # exec 子命令本身即 non-interactive；只读沙箱足够（sup 只调 gofer MCP，不写盘）。
    # codex 具体 flag 以你机器上的 codex-cli 版本为准（见 §6 真机核实）。
    args: [exec, "--sandbox", "read-only", "{{prompt}}"]
    # 新 agent 名不继承内置 codex 默认，显式补 system_inject（role 预设的 system_prompt 经此注入）。
    system_inject: ["-c", "developer_instructions={{system_prompt}}"]
    env:
      GOFER_AGENT_ROLE: supervisor     # ← 核心：让其 gofer mcp 子进程注册 role=supervisor
      # 若 codex 不向 mcp 子进程透传父 env（见 §6 gap①），改用 CODEX_HOME 兜底（§6 方案B）：
      # CODEX_HOME: /home/you/.codex-sup
    detect: { command: codex, args: [--version] }

# --- role 预设（可选，复用 system_prompt + 默认 project）---
roles:
  supervisor:
    agent: codex-sup                   # 指向 sup 专用 agent（带 role env）
    project: my-project1               # 默认项目（可被 -p 覆盖）
    system_prompt: |                   # 经 codex-sup.system_inject 注入；亦可改用 --prompt 直传
...
# --role supervisor 解析出 agent=codex-sup（带 role env）+ 注入 system_prompt
```
...
前置自检：
1. `gofer_list_agents` / `gofer agent ...` 能见 `codex-sup`，`detect` 通过。
...
- 可选小增强（非必须，便于少配一个 agent）：给 `RoleConfig` 加 `env map[string]string`，则 `--role supervisor` 可直接注入 `GOFER_AGENT_ROLE` 而无需单独的 `codex-sup` agent；当前用 per-agent `env` 已能零改代码达成，故仅记录为后续候选。
</content>
</invoke>
