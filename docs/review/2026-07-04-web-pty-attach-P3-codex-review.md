...
结论：**有问题，暂不建议收官进 P4**。加密封套本身未发现可绕过、可截断接受或 nonce 复用硬漏洞；但 relay 的 bounded close 超时路径没有把“封尾失败”传回 concrete sink，导致 `handlePtyConnect` 可能把未封尾/仍被 Close 卡住的 cast 文件 URI 写入 `pty_sessions`，并留下卡死 goroutine。判定：**阻断 0，高 1**。

## 重点逐条核对

1. **castrec framed AEAD：通过，未发现加密绕过。** `internal/castrec/envelope.go:49-57` 用 `hkdf.Key` 双返回值派生 master，再以 `fileID` salt 派生 per-file key；`internal/castrec/envelope.go:59-83` 的 nonce 是 `uint32BE(0)||uint64BE(frameIdx)`，在 per-file key 下无同 key nonce 复用；AAD 绑定 magic/version/fileID/frameIdx/isFinal/ptLen。`internal/castrec/envelope.go:290-325` 用 reader 侧递增 frameIdx 参与 nonce/AAD，EOF-before-final、final 后 trailing bytes、重排/删帧都会失败；攻击者不能用合法前缀加伪造 final 绕过（无 key 不能伪造 tag，复用原 final 会因 frameIdx 不匹配失败）。空会话 final frame 由 `internal/castrec/envelope.go:208` 的 `flushFrame(true)` 写空明文认证帧。`ct_len` 在 `internal/castrec/envelope.go:300-307` 限制为 `<=16KB+tag`，写端 `internal/castrec/envelope.go:153-160` 分块到 16KB，无整数溢出/越界读风险。
2. **relay close owner：有高风险。** close owner 已放到 `recordLoop defer r.finish()`，避免 `Write`/`Close` 并发：`internal/ptyrelay/relay.go:140-158`、`internal/ptyrelay/relay.go:262-285`。`Done()` 在 `finish` 后关闭：`internal/ptyrelay/relay.go:292-324`，P2 host wait 仍被 `hostCancelGrace` 兜底：`internal/runner/worker/runner.go:235-239`、`internal/runner/worker/runner.go:274-279`。但 `boundedCastClose` 超时只放弃等待，没有设置 sink 错误：`internal/ptyrelay/relay.go:305-314`，与 finalize 用 `sink.Err()` 判定 URI 是否可用的逻辑不闭环，见高风险 H1。`bytesIn` 按 `src.Write` 实际返回 `n>0` 计数：`internal/ptyrelay/relay.go:360-363`，只读 viewer 不计。
3. **T5 finalize：主体顺序正确，但受 H1 影响。** 单一 defer 中 `Close -> <-Done -> sink.Err -> Upsert(closed)`：`internal/httpapi/pty_connect_handler.go:156-179`，ctx 提前取消也会执行；Open 前失败关闭 sink 且不建行：`internal/httpapi/pty_connect_handler.go:123-132`；无 store 时 swallow：`internal/httpapi/pty_connect_handler.go:196-205`；未拨入无行语义由 `TestPtyConnectNoDialNoSession` 覆盖。cols/rows 从 pending `RelayEntry.Binding` 取，不从 nonce binding 取：`internal/httpapi/pty_connect_handler.go:103-106`，正确。
4. **T6 recording gate：通过。** 授权复用 `callerMayAttach`：`internal/httpapi/pty_recording_handler.go:37-46`，allow_empty 空 caller 不能读空 owner 录制的保守语义有测试；`filepath.Base(recording_uri)` 后再 `SafeJoinUnder(resultDir, base)` 重锚：`internal/httpapi/pty_recording_handler.go:62-77`；已无 remote-source 409 分支，worker-source 直接可下载：`internal/httpapi/pty_recording_handler.go:18-21`、`internal/httpapi/pty_recording_handler.go:82-90`。加密路径在 `WriteHeader(200)` 前完成 header + 首帧认证：`internal/httpapi/pty_recording_handler.go:105-119`；中途损坏只能断流并日志：`internal/httpapi/pty_recording_handler.go:126-130`。
5. **T3/T4 retention：通过。** `ExpireCastRecordings` 只清 `recording_uri` 并把 `encrypted=2`，保留行：`internal/jobstore/pty_sessions.go:121-185`；`PruneJobs`、`PruneWorkflows` 在同一事务内删除 job-owned `pty_sessions`：`internal/jobstore/prune.go:77-104`、`internal/jobstore/prune.go:220-233`；`Service.Prune` 对 cast 文件 best-effort `os.Remove`：`internal/job/prune.go:72-82`。prune loop gate 已含 cast enabled：`internal/serve/serve.go:291-300`。
6. **T0 config + G023：通过。** `Cast.Enabled` 默认 false，旧配置静默不录：`internal/config/model.go:454-462`；仅 enabled 且 TTL=0 时默认 24h：`internal/config/loader.go:270-275`；明文 TTL>24h fail-fast 仅在 enabled 时触发：`internal/config/loader.go:353-360`。加密 key 运行期 base64/hex 解码并要求 >=32B：`internal/config/loader.go:429-445`、`internal/serve/serve.go:331-343`。
7. **G024 leaf + 依赖方向：通过。** `go list -deps ./internal/ptyrelay` 在 `.cache` 下未命中 `internal/castrec|jobstore|job|config|httpapi|serve`；实现上 `ptyrelay` 只持 `{io.Writer; Close}` 接口：`internal/ptyrelay/relay.go:83-91`，concrete castrec 由 httpapi 注入：`internal/httpapi/pty_connect_handler.go:107-122`。
8. **资源关闭：有 H1 同源风险。** 正常路径 `Recorder.Open` 创建文件后，EncWriter/header/asciicast 失败都会关闭已打开文件：`internal/castrec/factory.go:70-87`；下载 `NewDecReader` 出错关闭文件、成功后 handler `defer dec.Close()`：`internal/castrec/factory.go:95-109`、`internal/httpapi/pty_recording_handler.go:105-110`。唯一硬风险是 `boundedCastClose` 超时后 goroutine 仍卡在 `sink.Close()`，fd 可能保持打开且 URI 仍可能入库，见 H1。

## 硬问题

### 高 H1. bounded cast close 超时不会标记 sink 失败，可能暴露未封尾 recording_uri 并泄漏 goroutine

位置：`internal/ptyrelay/relay.go:305-314`、`internal/httpapi/pty_connect_handler.go:156-162`、`internal/castrec/asciicast.go:106-119`。

失败场景：`recordLoop` 退出后进入 `finish()`，`boundedCastClose()` 在 goroutine 中执行 `r.cast.Close()`；如果 concrete sink 的 `Close` 在写 final frame / file close / Windows 文件系统或网络盘 I/O 上卡住超过 `castCloseGrace`，`boundedCastClose` 超时返回并 `close(done)`。随后 `handlePtyConnect` 的 finalize 被唤醒，只检查 `sink.Err()`；但 `sink.Err()` 只有 concrete `Close()` 返回错误时才会被设置，超时本身不会写入错误状态。结果是 closed row 可能保留 `recording_uri`，下载 gate 对 encrypted cast 会在首帧或后续帧认证失败返回 422/断流，plaintext cast 则可能被当作可用半成品直接 `ServeFile`。同时那条执行 `r.cast.Close()` 的 goroutine 会一直泄漏，fd 也可能保持打开。

这不是 AEAD 绕过，但违背 D-P3-1 “超时置 sink.Err，recording 视为失败”的实现契约，也违背本轮资源审查目标。建议修复方向：让 bounded close 的超时能写入 concrete sink 的失败状态，或让 ptyrelay 的 cast sink 接口包含窄的 timeout/fail hook；finalize 必须能识别 close timeout 并清空 `recording_uri`。

## 非阻断注意

- `internal/httpapi/server.go:400-402` 路由注释仍写“remote-source 409 mirrors artifact download”，但实现和测试已去掉 409：`internal/httpapi/pty_recording_handler.go:18-21`、`internal/httpapi/pty_recording_handler_test.go:167-183`。这是注释漂移，不是行为 bug。
- `go test ./...` 在本机 Windows 未全绿，失败集中在既有环境/测试脆弱面：TempDir cleanup 时 `stderr.log` 被占用、`sh`/Windows 路径断言、`internal/store` 的 `SafeJoinUnder("../x")` 测试等；P3 定向包测试通过，详见自证。

## 自证

- `go test ./internal/castrec/ -v -run Envelope`：通过；覆盖 round-trip、empty final、drop final、drop data frame、trailing-after-final、append frame after final、reorder、cross-file substitution、single-byte tamper、wrong key。
- `go build ./...`：通过（`GOCACHE=.cache/go-build`, `GOMODCACHE=.cache/gomod`）。
- `go vet ./...`：通过（同上）。
- `go list -deps ./internal/ptyrelay | Select-String 'internal/(castrec|jobstore|job|config|httpapi|serve)'`：无输出，leaf 边界成立。
- P3 定向：`go test ./internal/ptyrelay -run 'Finish|Bounded|Cast|Input|Registry' -v`、`go test ./internal/httpapi -run 'Pty(Connect|Recording)' -v`、`go test ./internal/jobstore -run 'Pty|Prune' -v`、`go test ./internal/job -run 'Cast|Prune' -v`、`go test ./internal/serve -run Cast -v` 均通过。
- `go test ./internal/worker -run E2EPtyCast -v` 在 Windows 下无测试运行，因为 `internal/worker/pty_cast_e2e_test.go:1` 是 `//go:build unix`。
- `go test ./...`：未通过；见上方非阻断注意，不能作为“全绿”收官证据。

## 收官判定

**有问题：阻断 0，高 1。** 加密(castrec)未发现可绕过/可截断接受/nonce 复用；但 `ptyrelay` close timeout 的失败状态和资源泄漏需要先修，修完后再跑本轮自证与定向回归。
