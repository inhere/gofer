...
- `source/gofer-wordmark.svg`：纯文字字标，字形已经矢量化，不依赖系统字体。
- `source/gofer-mark.svg`：无文字图形标，透明背景。
- `png/`：由 SVG 母版生成的常用尺寸透明 PNG。
- `web/public/favicon.svg`：针对 16–32px 显示单独简化的浏览器图标。
...
- 深色背景使用原版；浅色背景可将字标纸白色 `#E8E2D4` 替换为墨蓝色 `#0E1A24`，其余颜色保持不变。
