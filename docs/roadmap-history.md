# Gofer 增强路线 · 历史档案（2026-06-20 → 2026-09-18）

> **本文是归档**：2026-09-20 起路线图改为精简版 [`gofer-enhancements-roadmap.md`](gofer-enhancements-roadmap.md)（只列功能点与状态，细节链接到各 design / plan / runbook）。
> 这里原样保留旧版全文——完整修订记录（v1.0–v1.30）、旧 `E` 编号映射、每个条目当年的构想/权衡/落地细节——供追溯，不再更新。
...
| v1.27 | 2026-07-02 | claude | **Web 控制台 v3 P1 只读观察层落地（SUPMODE + host codex）**：8 提交 `1a6d019`(worker 节点透视 4 层)/`d873f13`(/v1/stats+jobs offset)/`4def6fa`(types+client)/`8d9dbbb`(Dashboard)/`c1016b6`(Drivers+Inbox+路由)/`d44a82e`(导航壳+只读铃铛)/`a3841f0`(Board 分页/徽标·Cluster·Agents·JobDetail)/`4438fb2`(running rendered_command)。**WEB-06/07/08 ✅**、**presence/inbox 展示 ✅(#6/#7)**、worker 节点 projects/agents 透视 ✅。全程 go test/pnpm build 亲验绿 + 生产 bake(dist/gofer.exe) 验证。plan `plans/2026-07-02-web-console-v3/P1-readonly-plan.md`（checkbox 全勾）。**WEB-09 写层应答=P2**（前置身份分级）。待用户主机部署上线 + 浏览器眼检。 |
| v1.28 | 2026-07-02 | claude | **Web 控制台 v3 P2 交互写层（WEB-09）落地（SUPMODE + host codex）**：5 产物 `c316cda`(P2.1 `answered_by`=服务端盖认证 caller_id 审计归属,不触 agent 派生作答闸+`interaction.punted` 事件)/`2d8dfbe`(P2.2 可选 `can_answer` 能力位闸,opt-in+fail-fast 防锁死)/`063011e`(P2.3 EscalationBell 内联 choice/confirm 应答+punt,per-item pending/error)/`2824856`(P2.4 JobDetail punt+answered_by 展示)+ 本 runbook。**WEB-09 ✅**。全程容器 go test 全绿(证实 codex 报的失败为 Windows host 环境特有) + 独立 host pnpm build 亲验绿。plan `plans/2026-07-02-web-console-v3/P2-interaction-write-plan.md`（checkbox 全勾）+ runbook `runbook/2026-07-02-web-interaction-answer-runbook.md`。待用户主机部署上线 + 浏览器眼检写闭环。 |
| v1.29 | 2026-07-03 | claude | **WEB-04③ 配置写层 V1 落地（SUPMODE + host codex）**：5 产物 `7900d1d`(C1 `can_admin` 能力位+`require_admin_capability` 闸,仿 can_answer+fail-fast)/`96920d1`(C2 `GET /v1/config` 脱敏全量只读,secret→`*_set`、env 只列 key 名,10 项 secret 泄露断言)/`ca4c3ed`(C3 项目 create/update/delete 写端点,`can_admin` 闸+引用校验拒非法不落盘+Registry 落全局 live 生效+审计 slog)/`efa3844`(C4 `Config.vue` 脱敏总览+项目编辑器)+ runbook。**WEB-04③ V1 ✅**（scope=projects CRUD+脱敏只读；server/secret/governance 编辑 V1.1 待,因热重载盲区+secret 写策略）。容器 go test 全绿(29 pkg) + 独立 host pnpm build 亲验绿。design `design/2026-07-03-web-config-write-design.md`(§12 决策拍板) + plan `plans/2026-07-03-web-config-write-plan.md`(全勾) + runbook `runbook/2026-07-03-web-config-edit-runbook.md`。**IA 精修**(`ee56fa3`,用户反馈)：项目编辑器并入 `Projects` 页(读+写)、`Config.vue` 收敛为纯只读系统配置总览、菜单从 舰队 组独立成 `⚙ 设置` 入口(与内容分组分隔)。待部署上线 + 眼检。 |
| v1.30 | 2026-07-07 | claude | **WEB-03 浏览器 pty 交互全收官 ✅**（回填：P0-P4 SUPMODE + host codex，2026-07-04~06）：P1 serve 侧协议/admission/relay/attach ws · P2 worker 端到端 pty attach（真 ptmx + 第二条 pty ws 字节泵 + 三处收敛取消协议）· P3 cast 加密录制 + `pty_sessions` 表 + retention + `/pty/recording` gate · P4 前端 `AttachTerminal.vue`(xterm)+JobDetail 接入+Sessions 视图+e2e 全矩阵。叠加 **local interactive attach**（`job --interactive` 直连 local ptmx + 交互会话免超时，bd `h-aii-6nrg`）+ **session/terminal UX 打磨**（Sessions 进终端/捕获展示 session_id/per-session 录制开关，bd `h-aii-2t3j`）。技术债 `h-aii-4vqw`（Windows 句柄清理）已修 closed（`83b15be`）/ `h-aii-wag4`（writeFrame reconnect 语义）源码加 `TODO` 标记（`d1be9a1`）保持 open。总纲 [`plans/2026-07-03-web-pty-attach-plan.md`](plans/2026-07-03-web-pty-attach-plan.md)(v0.2 P1-P4 全勾) + design v0.8。待主机部署 + 浏览器眼检 + gofer 独立仓 push。 |
...
| v1.31 | 2026-09-06 | inhere+claude | 新增 **SESS-01 终端 agent 会话 ↔ web 消息中继**（Stop hook `decision:block` 把 web 回复注入同一终端会话；server 侧会话注册表+relay 开关；执行体内置为 `gofer hook claude|codex`，`gofer init hooks` 一键装配 `.claude/`/`.codex/`；turn 复用决策通道）。design [`design/2026-09-06-agent-session-relay-design.md`](design/2026-09-06-agent-session-relay-design.md)，取代 handoff 设计 §11 的"裸终端不可远程作答"结论，吸收其 §5 adopted_sessions。**同日 T1-T6 全部落地**（jobstore/sessionrelay/httpapi/hookrelay/commands/web/skill+runbook），容器内 Claude Code 真机 e2e PASS；Codex 待主机验证。 |
...
| WEB-06 ✅ | **Web 控制台 v3 导航 / 信息架构重构（壳层）** | 中 | 中 | **P1 已落地**（v3 P1.4，commit `d44a82e`）：顶栏分组导航（Home / 观察 / 舰队）+ 新增 Drivers 入口 + rail drivers 计数 + 只读 EscalationBell/Toast（跨页 pending 感知，无应答=P2）。design [`design/2026-07-02-web-console-v3-design.md`](design/2026-07-02-web-console-v3-design.md)。**Web 控制台 v3** 主题（WEB-03 pty / WEB-04③ config / WEB-09 应答=P2 写层）。 |
| WEB-08 ✅ | **job 列表 / 详情体验补强** | 中 | 中 | **P1 已落地**（v3 P1.7 `a3841f0` + P1.7b `4438fb2`）：列表 offset 分页 + role/channel 行内徽标（时间列 hh:mm:ss 原已有）；详情 **running 即显 rendered cmd**（P1.7b 后端在 running 转换处 persist `rendered_command` + 前端脱离 outcomes 门控）· STDOUT/STDERR tab + ANSI（原已在 LogTape，本期验证）。 |
...
| SESS-01 ✅ | **终端 agent 会话 ↔ web 消息中继（session relay）** | 高 | 中 | **已落地**（2026-09-06，T1-T6，Claude 真机 e2e PASS，Codex 待主机验证；runbook [`runbook/session-relay.md`](runbook/session-relay.md)）。人离开电脑后，终端 Claude Code / Codex 会话停下等人的那条消息进 web，web 回复经 **Stop hook `{"decision":"block","reason"}`** 注入**同一**终端会话继续（不开 pty、不重开会话）。server 侧 `agent_sessions` 注册表 + 会话级 relay 开关（终端 CLI / web / 回复 `/off` 三入口，web 可补开、下一次 Stop 生效）；turn 复用 `plan_decisions`（铃铛/卡片/过期全复用）；执行体为 gofer 二进制 `gofer hook <agent>`，`gofer init hooks` 合并写 hooks 配置。硬边界：已空闲提示符的会话无 hook 可注入（可选 tmux 兜底）。design [`design/2026-09-06-agent-session-relay-design.md`](design/2026-09-06-agent-session-relay-design.md)。跨①②轴。 |
...
| WEB-09 ✅ | **全局交互通知 + Web 应答（写/交互层）** | 高 | 中-大 | **P2 已落地**（v3 P2.1-P2.4，commit `c316cda`/`2d8dfbe`/`063011e`/`2824856`）：EscalationBell 就地 choice/confirm 应答 + JobDetail punt + `answered_by` **服务端盖认证 caller_id**（防伪，不触 agent 派生作答闸）+ 可选 `can_answer` 能力闸（opt-in，默认放行兼容）。实现 MCP-01"升级人→Web 应答"面 + 通用交互应答。前置鉴权分级已定（design §13：caller_id 归属 + 独立 operator token + can_answer 闸）。runbook [`runbook/2026-07-02-web-interaction-answer-runbook.md`](runbook/2026-07-02-web-interaction-answer-runbook.md)。来源：用户反馈 2026-06-30。待用户主机部署上线 + 浏览器眼检。 |
...
| OBS-07 🚧 | **IM 连接（钉钉/飞书）** | 高 | 大 | **(a) 已落地**（2026-09-07）：webhook 加 `kind` 厂商适配（dingtalk/feishu 渲染各自消息 JSON + 各自签名：钉钉进 query、飞书进 body），复用既有投递队列/退避/审计/出站校验，**不新建 channel 实体**；`event_deliveries` 加可空 `body`/`event_type` 走预渲染路径，使非 job 事件（`session.waiting`，来自 SESS-01）也能复用这条队列；新增 `server.web_base_url` 让消息带可点链接。runbook [`runbook/im-notification.md`](runbook/im-notification.md)。(b)(c) 未做。<br>—— 原始构想 —— 跨①②③轴。**拆三层**：(a) 出站通知接 IM（复用 OBS-03，**最便宜先行**）；(b) 入站提交（IM 消息→job，新入口，回调验签 + IM 用户映射 caller_id）；(c) 交互应答（pending_interaction→IM 卡片→回复→续跑）。平台差异需 adapter。鉴权接 OBS-06 caller。 |
| WEB-04 🚧 | **节点拓扑 + 配置管理（Web）** | 中 | 中-大 | **① 拓扑图 + ② 节点面板（只读）已落地**（web v2 只读层 P4，commit `de4288a`）：`Cluster.vue` SVG 星型拓扑（hub+worker+peer+local，色=状态/心跳脉冲）+ 点击节点面板（worker 心跳/in-flight/labels、peer latency、local projects 概览），复用 `/v1/runners`+`/v1/projects` 无新后端；不画项目→节点边（D2，server 不知 worker projects）。**③ 配置查看/编辑（写层）V1 已落地**（2026-07-03，commit `7900d1d`/`96920d1`/`ca4c3ed`/`efa3844`）：`GET /v1/config` 脱敏全量只读（secret→`*_set` bool、env 只列 key 名）+ 项目 create/update/delete 写端点（走 `Registry` 落全局 live 生效，准入合规 D2，引用校验拒非法不落盘）+ `can_admin` 能力闸（opt-in 默认关+fail-fast）+ `Config.vue` 总览/编辑器。**server/callers/token/governance/agents 编辑 V1.1 待**（热重载盲区 + secret 写策略）。design `design/2026-07-03-web-config-write-design.md` + runbook `runbook/2026-07-03-web-config-edit-runbook.md`。跨①③轴。 |
...
| WEB-07 ✅ | **Dashboard 首页（聚合统计 + 状态总览）** | 中 | 中 | **P1 已落地**（v3 P1.2 后端 `d873f13` + P1.5 前端 `8d9dbbb`）：新增 `GET /v1/stats` 轻量聚合（jobs by-status/workflows(占位)/schedules/runners/drivers/escalations/projects）+ Dashboard 纯数字块首页（`/`→/dashboard，5s 轮询）。 |
