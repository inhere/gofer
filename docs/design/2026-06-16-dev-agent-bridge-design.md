...
> "给其他项目使用" 指任意被 Docker 挂载、且在本工具配置中显式登记的目录 / 项目；不限于当前 `workspace` 工作空间内的子项目。
...
agent-bridge project add workspace --host-path D:/projects/workspace --container-path /workspace
agent-bridge agent list
agent-bridge job run --project workspace --agent codex --cwd tools/dev-agent-bridge "检查当前设计"
...
CLI 使用 `github.com/gookit/gcli/v3`，整体风格参考 `D:\tools\gcli`：
...
```bash
agent-bridge job run --project workspace --agent codex --cwd . "检查当前设计"
...
projects:
  workspace:
    host_path: D:/projects/workspace
...
```json
{
  ...
  "status": "done",
  "exit_code": 0,
  "cwd": "D:/projects/workspace/tools/dev-agent-bridge",
