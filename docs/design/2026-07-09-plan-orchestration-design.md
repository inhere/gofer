...
**归组键命名（v0.2 锁定）= `plan_id`**（非泛化的 `group_id`）。理由：
- 域概念具体清晰——"一个完整计划"就是 plan；`group_id` 语义模糊（"按什么分组？"）易招致范围蔓延。
- plan 是唯一的一等归组概念：一个 plan 可同时含独立 job、workflow、todo；未来真需要另一维度分组，**显式加新键**（如 `batch_id`）而非重载一个万能 `group_id`。
- **todo↔job 是另一条独立关系**，不影响归组键命名：todo 通过 `todo.job_id` 绑到某次 job 执行（进度联动），通过 `todo.plan_id` 归入计划；job 的归组仍是 `job.plan_id`。三者正交、互不耦合。
...
- **两种 todo**：① 纯待办（`job_id=NULL`，普通清单项）；② **绑定 job 执行**（`job_id` 指向某次 job，job 终态可联动 done——回应"todo 也可能关联到 job 执行"）。todo 始终属某 plan（`plan_id`），但**是否绑 job 独立可选**。
...
**已定（v0.2 评审）**：归组键 = **`plan_id`**（理由见 §4）；todo 支持纯待办 + 绑 job(`job_id`) 两种。

**待确认**：
1. `gofer_run_job` 直接加 `plan_id` 参数（提交即归组）vs 单独 `attach_job`——两者都要？（倾向：都要，run_job 带参最顺、attach 补挂已有 job）
2. plan 与 workflow 是否交叉引用（一个 workflow 整体归入某 plan）——`workflows` 表加 `plan_id`？（倾向：需要）
3. todo 绑 job 的进度联动：job 终态 done 是否自动勾选 todo？（倾向：可勾但不强制自动）
4. 是否需要 plan 级权限/owner 校验（谁能 attach/archive）。
5. 分期按 §11 先做 P1+P2 打通"编排即归组"最小闭环？
