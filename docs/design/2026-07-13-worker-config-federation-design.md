...
| v0.8 | 2026-07-14 | Claude | **P3 计划的对抗式审查回灌本文（3 条 BLOCKER，均有代码证据）**：<br>① 🔴 **迁移开关改挂 worker 的 `roots`，不再挂 server 的 proto 版本（新增 D7）** —— §6.3/§9 的「**发布纪律：server 必须先升**」建立在「worker 一旦是 v4 就以 Policy 为权威」这个**错误的开关设计**上。核实：live 两台 worker.yaml **都有 `projects`、都没有 `roots`** ⇒ 那样的开关会在**换二进制那一刻**把两台 worker 双双打到 0 project（roots 为空 ⇒ 全部 `path_outside_roots`）。**改为：`roots` 非空才进 POLICY 模式**（opt-in）；`roots` 空 + `projects` 非空 = **LEGACY**（忽略 Policy，逐字保持今天的行为）。⇒ 换二进制**零行为变化**；迁移 **per-worker、可回滚**；**「server 必须先升」作废**（roots 的有无与 server 版本正交）。<br>② 🔴 **§7.2 的「改一条策略」漏了 web 写路径**：`project.Registry.Add` **原地改**那个 `*config.Config`（与 `core.Build` 里的是同一个指针），**不走 `ReloadWith`**（`POST/PUT/DELETE /v1/projects` 全走它）⇒ web 控制台改完 **Policy 永不重推**；且 Rev 只在 `ReloadWith` 里 +1 ⇒ **两份不同内容的 cfg 共用一个 Rev** ⇒「旧 Rev 丢弃」建立在假前提上。⇒ **所有改 cfg 的路径必须收进唯一写入口**（copy-on-write + Rev++ + 推送，都放进 `ReloadWith`）。<br>③ 🔴 **Policy 推送不得在 `Registered` ack 之前插帧**：hub 的 `reg.Put` 在写 ack **之前**，而 worker 握手只读一帧、`As[Registered]` 既不校验 `env.Type` 也吞掉 decode error ⇒ policy 帧被解成 `Accepted=false, Reason=""` ⇒ worker 报「注册被拒（原因为空）」→ **重连风暴**。⇒ `Put` 挪到 ack 之后 + worker 断言帧类型。<br>④ **D6 投影表补两个字段**：`MaxConcurrentJobs`（不投 = 恒 0 = **无限并发**）、`CaptureDiff`（不投 = nil = **默认开**）。<br>⑤ **`policy_pending` 不得新增拒绝路径**（只换错误文案）：Rev 每次重连清零 ⇒ 每次网络抖动都重进 pending，而 worker 此刻仍持有完全有效的上一份 cfg；硬拒会打挂在跑的 workflow / cron。<br>⑥ §7.2 删掉**不存在的** `gofer serve reload` 引用（同 v0.7 删 `gofer worker show`）。 |
...
- **D4 project 放置由 roots 推导，worker 端零 project 配置**（v0.3，推翻 v0.2 的 Q4）
  ...
  - 迁移：见 **D7**（v0.8 改写——原文写的「P3 直接转只读 + 告警」会在换二进制那一刻打死现网 worker）。

- **D7 迁移开关挂在 worker 的 `roots` 存在性上（v0.8 新增，推翻 v0.6/v0.7 的「server 先升」纪律）**
  - **原方案的错（对抗审查坐实）**：v0.6/v0.7 隐含的开关是「**worker 是 v4 ⇒ Policy 权威**（哪怕 Projects 为空）」，于是必须配一条**发布纪律**「server 必须先升」。但现网 worker.yaml **有 `projects`、没有 `roots`**（`grep -c "^roots:" <live worker.yaml>` → 0）⇒ 换成 P3 二进制的那一刻：roots 为空 → 每个 project 都 `path_outside_roots` → `cfg.Projects={}` → **worker 一个 job 都跑不了**。**纪律再严也救不了**，因为它要求的顺序（server 先升）根本不解决这个问题——问题出在 worker 自己的配置形态上。
  - **开关（定稿）**：
    ```txt
    len(roots)==0 && len(projects)>0  → LEGACY：忽略 Policy，继续用 worker.yaml 的 projects（= 今天的行为，逐字不变）
                                        仍回 Applied{Rev, Caps(本地), Degraded:[legacy_local_projects]}，
                                        让 server 清 policy_pending、并在 /v1/meta 一眼看出「这台还没迁」
    len(roots)>0                      → POLICY：Policy 权威，忽略 worker.yaml 的 projects（非空则告警）
    len(roots)==0 && len(projects)==0 → EMPTY：醒目告警，本 worker 不会接到任何 job（今天这种 worker 本来也跑不了东西）
    ```
  - **买到了什么**：① **换二进制 = 零行为变化**（"现网零破坏"不再是嘴上说说）；② 迁移变成 **per-worker、可回滚**（删 `roots`、留 `projects` 即回到今天）；③ **「server 必须先升」这条脆弱纪律作废** —— `roots` 的有无与 server 版本**正交**，回滚 server 到 v3 也安全（只要 worker.yaml 还留着 `projects`）。
  - `Registered.ProtocolVersion` **仍然要**，但用途降级为**告警文案的精度**：POLICY 模式下 0 个 project 时，`<4` ⇒「server 太旧，不支持策略下发」；`>=4` ⇒「server 没给本 worker 推任何 project，检查 `allowed_runners`」。
  - 代价：`projects` 段在 P3 **不会真正消失**（LEGACY 仍完全生效），真正移除留到下一个版本。**这是刻意的**——它是本期唯一的护城河。
...
- **D6 Policy → worker 本地执行配置的投影**（v0.5 新增；v0.6 按 P2 事实重写）
  ...
  - **投影规范**（worker 收到 Policy 后构造本地 `config.Config`，整份**原子替换**）：
    ...
    | **`MaxConcurrentJobs`**（v0.8 补） | **原样透传**。`job/submit.go` 的 `s.semaphore(req.ProjectKey, proj.MaxConcurrentJobs)` 读它 —— **不投 ⇒ 恒 0 ⇒ 无限并发**（今天 worker.yaml 里的 `max_concurrent_jobs` 会**静默失效**） |
    | **`CaptureDiff`**（v0.8 补，`*bool`） | **原样透传**。`job/outcomes.go` 的 `shouldCaptureDiff` 读它 —— **不投 ⇒ nil ⇒ 默认开**（`capture_diff:false` 会**静默复活**） |
    | **agent 定义** | **不投影、不拼装。** 见下方 ⚠️ |
    | `Storage`（result / exchange 子目录） | 取 worker 本地 `storage` 配置（本机事实，不由 server 定；`workerConfigToConfig` 已经这么做）。⇒ `ExchangeSubdir`/`ResultSubdir` **不下发**（空值经 `ResolvedExchangeSubdir`/`ResolvedResultSubdir` 回落 worker 本地默认，已核实） |
    | `ContainerPath` / `DefaultAgent` | **不下发**（已核实：worker 无 `path_view` ⇒ `ExecPath` 恒取 `HostPath`；`DefaultAgent` 不在 worker 的执行链上，只有 CLI 展示 / httpapi / registry.Validate 读它） |
...
> ⚠️ **v0.8（D7）：上面是**目标态**，不是 P3 的强制态。** `roots` **非空**才进 POLICY 模式（`projects` 被忽略）；`roots` 为空而 `projects` 非空 = **LEGACY**（忽略 Policy，逐字保持今天的行为）。迁移是 **per-worker opt-in、可回滚**：加 `roots` → reload → 核对 `/v1/meta` 的 project 集 → 一致后再删 `projects`。**换二进制本身零行为变化。**
...
> ⚠️ **v0.8 重写**：矩阵的关键维度不是「worker 是不是 v4」，而是 **worker.yaml 的形态**（D7 的开关）。

| 组合 | worker.yaml 形态 | 预期 |
|---|---|---|
| v4 server + v3 worker | 任意 | 不下发 Policy（`PolicyMinProtocolVersion` 闸），worker 继续用它本地的 `projects` 段 |
| v4 server + v2 worker | 任意 | 同上（v2 连 reload 都没有，只是能连能跑） |
| **v3 server + v4 worker** | **LEGACY**（有 projects、无 roots） | **完全正常** —— Policy 本来就不来，worker 用本地 projects。**D7 之后这一格不再危险**（原文说它是"最危险的一格"，那是错误开关设计的产物） |
| **v3 server + v4 worker** | **POLICY**（有 roots、无 projects） | 0 project + **醒目 `slog.Error`**（"server 不支持策略下发(proto<4) → 本 worker 不会接到任何 job"，靠 `Registered.ProtocolVersion` 才能说准），**不崩溃、仍在线**。回滚 = 删 roots / 恢复 projects |
| v4 server + v4 worker | 两种形态 | LEGACY 走本地 projects；POLICY 走下发。**全功能在 POLICY 形态** |
| 多 URL 混合（新旧 server 并存） | POLICY | 轮到旧 server ⇒ 上一行的告警降级；轮回新 server ⇒ register 时重新拿到 Policy 并应用。**Rev 状态必须能承受这种来回**（见 Q3/Rev 语义：Rev per-connection，register 时清零） |

> ~~**发布纪律：server 必须先升。**~~ **v0.8 作废**（D7）：`roots` 的有无与 server 版本正交，升级顺序不再影响正确性。

## 7. 关键流程

### 7.1 worker 接入（P3 之后的目标态）

> **v0.8**：下面是 **POLICY 模式**（worker.yaml 有 `roots`）的流程。LEGACY 模式（有 `projects`、无 `roots`）下 worker **忽略 Policy**、继续用本地 projects，只回一个带 `legacy_local_projects` 标记的 `Applied` 让 server 清 pending（D7）。
...
```txt
三条等价入口（v0.8：三条都必须重推 —— 见下方 ⚠️）：
  a) 编辑 server config → kill -HUP <serve pid>        （`gofer serve reload` 不存在，别写进文档）
  b) gofer project add X --path <逻辑路径> --agent tty-claude
  c) web 控制台「新增/编辑 project」（POST/PUT/DELETE /v1/projects）
  → server 换代（Rev+1）并重推 Policy 给所有在线 worker
...
⚠️ **v0.8（BLOCKER，代码事实）**：今天只有 (a) 会走 `core.ReloadWith`。(b)/(c) 走的是 `project.Registry.Add`，它**原地改**那个 `*config.Config`（与 `core.Build` 里的是同一个指针）**并直接落盘**——**既不 Rev++、也不触发任何 reload**。⇒ 本设计宣传的头号收益（web 上改白名单 / 开 exec / 开 pty，worker 自动跟上）**在这条路径上根本不成立**；更糟的是 **Rev 不再是配置代次**（两份不同内容的 cfg 共用同一个 Rev），worker 侧「rev ≤ lastRev 就丢弃」建立在假前提上。

⇒ **P3 必须把所有改 cfg 的路径收进一个唯一写入口**（copy-on-write 出新 cfg → 原子换指针 → **Rev++** → **推 Policy**，全部放进 `ReloadWith` 内部），让"忘记推送"在结构上不可能发生（而不是靠纪律）。顺带解掉 `Add` 原地写 map 与 `job.Service` 并发读的**既存 data race**。
...
| **P3** | 📋 **计划 v0.2（对抗审查后重写）见 [`2026-07-15-worker-policy-push-plan.md`](../plans/2026-07-15-worker-policy-push-plan.md)** | Policy 下发（**proto v4**）：**新建** worker 的 `roots` / `guards` 字段；**唯一配置写入口**（copy-on-write + Rev++ + 推送，覆盖 SIGHUP / CLI / web 三条写路径）；server 按 runner 可达性算推送目标（D4′）；Policy 随 `Registered` ack 下发（Q7，**ack 之后才 Put，防插帧**）；worker 按 **D7 的模式开关**（`roots` 存在性）决定 LEGACY / POLICY；POLICY 下按 roots 最长前缀映射自筛 → 按 D6 投影出 `config.Config` → `core.ReloadWith` 应用 → `Applied{Caps,...}` 回报；worker 落只读 policy 缓存文件（供 worker 机器上的 CLI 读）；`worker-only project` 配置概念退役（placeholder 代码路径复用）。**`projects` 段本期不删**（LEGACY 仍生效，D7 的护城河）。**不含** `Policy.Agents` / `allow_custom_agents`（Q6 砍） | P1,P2 |
...
**~~版本闸拆分是 P1 的第一个任务~~ ✅ 已做完**（`MinProtocolVersion=2` 与 `CurrentProtocolVersion=3` 已分离，注册闸读 `Min`）。**P3 把 `Current` 提到 4 是安全的**——不会踢掉任何 v2/v3 worker。

> ⚠️ **v0.8 更正**：原文接着写「P3 真正的升级风险是 worker 先升、server 还旧 → 停摆，所以发布纪律是 **server 先升**」。**这条纪律已作废（D7）**——它建立在"worker 是 v4 就以 Policy 为权威"这个错误开关上。真正的风险是**换二进制那一刻**（与升级顺序无关）：现网 worker.yaml **有 `projects`、无 `roots`**，一旦以 Policy 为权威，roots 为空 ⇒ 全部 `path_outside_roots` ⇒ 双双归零。**开关改挂 `roots` 存在性**后：换二进制零行为变化，迁移 per-worker 可回滚，升级顺序不再影响正确性。worker 在「POLICY 模式但从未收到过 Policy」时仍打醒目告警（靠 `Registered.ProtocolVersion` 区分"server 太旧"与"策略为空"）。
...
### v0.8 补充核实（C20-C25，P3 计划的对抗审查发现；三条 BLOCKER 全在这里）

| ID | 断言 | 核实命令 | 结论 / 对 P3 的含义 |
|---|---|---|---|
| C20 | **live worker.yaml 有 `projects`、无 `roots`** | `grep -c "^roots:" <live worker.yaml>`（→0）<br>`grep -n "^projects:" <live worker.yaml>`（→命中） | ✅ ⇒ **「worker 是 v4 就以 Policy 为权威」的开关会在换二进制那一刻把现网 worker 打到 0 project**（roots 为空 ⇒ 全部 `path_outside_roots`）。**D7 的直接依据** |
| C21 | **`project.Registry.Add` 原地改 cfg，不走 `ReloadWith`；web 写路径全走它** | `grep -n "func (r \*Registry) Add" -A 13 internal/project/registry.go`<br>`grep -n "project.NewRegistry" internal/core/core.go`（同一指针）<br>`grep -n "s.projects.Add\|s.projects.Remove" internal/httpapi/project_handler.go`<br>`grep -rn "cr.Reload(" internal/serve/serve.go`（SIGHUP 是唯一 reload 调用点） | ✅ ⇒ ① web 改 project **Policy 永不重推**（§7.2 的头号收益不成立）；② **Rev 不是配置代次**（两份不同内容的 cfg 共用一个 Rev）；③ `Add` 原地写 map 与 `job.Service` 并发读 = **既存 data race**。⇒ 必须收进唯一写入口 |
| C22 | **hub 的 `reg.Put` 在写 ack 之前；worker 握手不校验帧类型、吞 decode error** | `grep -n "h.reg.Put(wc)" -A 12 internal/wshub/hub.go`<br>`grep -n "reg, _ := wsproto.As" -B 3 -A 6 internal/worker/client.go`<br>`grep -n "func As\[" -A 8 internal/wsproto/envelope.go` | ✅ `As[Registered](policy帧)` → `err=nil, Accepted=false, Reason=""` → worker 报「注册被拒（原因为空）」→ 退避重连 → 再撞 = **重连风暴**。⇒ `Put` 挪到 ack 之后 + worker 断言 `env.Type` |
| C23 | **ack 用包级 `writeEnvelope(ctx, conn, …)`，绕过 `wc.writeMu`** | `grep -n "4) Ack" -A 5 internal/wshub/hub.go`<br>`grep -n "writeMu" -B 2 -A 6 internal/wshub/registry.go` | ✅ Put-before-ack 下，ack 与被推送的帧是同一连接上的**两个并发 writer**（coder/websocket 明确禁止）。C22 的修法顺带解掉；ack 改走 `wc.writeFrame` 更稳 |
| C24 | **D6 漏了两个 worker 真的会读的字段** | `grep -n "s.semaphore(req.ProjectKey" internal/job/submit.go`<br>`grep -n "proj.CaptureDiff" -B 4 internal/job/outcomes.go` | ✅ `MaxConcurrentJobs` 不投 ⇒ 恒 0 ⇒ **无限并发**；`CaptureDiff` 不投 ⇒ nil ⇒ **默认开**。两者今天在 worker.yaml 里都是生效的，会**静默失效** |
| C25 | **pending 硬拒会打挂 workflow / cron** | `grep -n "e.ops.Submit(req)" -B 4 -A 3 internal/job/workflow/advance.go`<br>`grep -n "cr.Jobs.Submit(req)" -B 4 internal/serve/serve.go` | ✅ 任一 fan 的 submit 失败 → `return err` → 整条 workflow fail；cron 同理。而 Rev 每次重连清零 ⇒ 每次网络抖动都重进 pending ⇒ **`policy_pending` 只能换错误文案，不能新增拒绝路径** |
