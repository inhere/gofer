...
- **`MaybeDispatchTodo` 每写入路径一次、幂等**：写完就调，它自己重新读 todo/jobs 做判定。**没有 sweeper**：server 重启不补派（设计决策 5），`serve` 里没有这一段代码——所以「重启后 plan 里躺着的 ready 项」不会自己跑起来。**已知边界**：判定不是原子的（「无活跃 job」是读后决策，没有 claim/锁）。两个**并发**写入（例如 web 与 CLI 同时点派发）理论上可以各起一个 job——这与设计一致（判据是"没有活跃 job"而非"至多一个 job"），且第二次派发落在用户明确的重复点击上，故本批不引入锁；要杜绝就再加一层 `UPDATE plan_todos ... WHERE status='ready'` 抢占（留给后续）。
