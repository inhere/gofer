...
| **mcp-client runner** | **未做**、方向相反：bridge 反过来当 MCP **客户端**去调"本身是 MCP server 的容器内能力" | 执行位置(规划) |
...
- **`mcp-client runner`（未做）**：bridge = MCP **client**，bridge 调别的 MCP server（执行位置，规划）。
...
| `mcp-client runner`（bridge 当 MCP 客户端） | 🔲 规划，未设计 |
