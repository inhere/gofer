...
| # | 约束 | 影响 | 建议 |
|---|---|---|---|
| C1 | **内存 job 表 + `jobs.jsonl` 无界增长**：`Service.jobs` 永不驱逐，索引只追加 | 长跑 server 内存/磁盘持续涨 | **方向已定：SQLite（modernc 纯 Go）做元数据/索引/交互 store、日志仍文件、内存仅留 live job** → 设计 [`2026-06-18-sqlite-store-design.md`](./2026-06-18-sqlite-store-design.md)（SP1–SP5） |
