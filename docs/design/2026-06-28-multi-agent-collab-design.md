...
> 注：mcp 工具前缀已统一更名（bridge_* → gofer_*，见 supervisor-routing plan P0），本文工具名以 gofer_* 为准。
...
- **L2 身份/信箱(E36)**：`agent_presence` + `messages` 两表 + 注册/心跳/收发原语（mcp `gofer_register`/`gofer_poll_inbox`/`gofer_post_message` + 对应 `/v1` 端点 + client 方法）。
- **L3 角色(E35)**：config `roles:` 段 + per-agent `system_inject` argv 模板 + `JobRequest.Role` 解析（`job run --role` / `gofer_run_job(role=)`）。
...
协作:  A gofer_post_message(to=B, kind=task) ─▶ messages ─▶ B gofer_poll_inbox 取到 ─▶ B gofer_run_job 执行
作答:  job 提问 ─▶ interactions(pending) ─▶ [L4 answerer] 白名单? 自动答 : 升级(post escalation 到 owner inbox) ─▶ driver 取到 ─▶ gofer_answer_interaction
```
...
| **internal/mcpserver** | Backend 接口扩 5 方法 + 5 个新 `gofer_*` 工具 + view 投影 | 沿 E28 双实现：local 直调 presence/job，client 转发 `internal/client` |
...
```
driver A 启动协作 → gofer_register(name="reviewer-A", role="reviewer", project="cv")
...
循环:  gofer_poll_inbox(agent_id, agent_token) → 校验句柄 → 取 unread + 刷新 last_seen → 处理 → (隐式心跳)
...
```
A: gofer_post_message(to="reviewer-B"|"role:reviewer", kind="task", body="审下 PR#12", ref="job:..."?)
   → messages 入库(unread, from=A.agent_id)
B: gofer_poll_inbox(B.agent_id) → 取到 task → B 自行决定: gofer_run_job(...) 执行 / 直接在自身上下文做
   → (可选) B gofer_post_message(to=A, kind="note", body="已完成 → job xyz", ref="job:xyz") 回执
```
...
策略判定(每条 pending):
  ...
  │     → driver 轮询取到 → gofer_answer_interaction(...) 人工/agent 作答
...
- **可选高级 = 派生监督 driver agent**：把"难答的 pending"经 escalation 投给一个**监督 driver agent**（它就是 §8.1 注册的 driver，role=`supervisor`），由它 reason 后 `gofer_answer_interaction`——即"用一个 agent 答另一个 job 的提问"。**复用 L1/L2，无新执行模型**；硬约束：只见白名单内、最大轮次、E17 配额、E13 审计。本文给出位置，细化留 plan/后续。

### 8.5 角色化运行（E35）

```
job run --role reviewer  (或 gofer_run_job(role="reviewer"))
...
**常驻 supervisor agent（§8.4 派生形态）**：内置规则 answerer 只可靠答 "choice + options"；要答更复杂 pending，常驻一个 role=`supervisor` 的 driver agent（§8.1 注册）→ `escalate_to` 指向它 → 它轮询 inbox 取 escalation → reason 后 `gofer_answer_interaction` 作答。硬约束：白名单内 + `max_rounds_per_job` + E17 配额 + E13 审计。
...
### 新增 mcp `gofer_*` 工具（Backend 接口扩 5 方法，沿 E28 双实现）

| 工具 | → 端点 | local / client |
|---|---|---|
| `gofer_register` | `POST /v1/agents/register` | local 直写 presence；client 转发 |
| `gofer_poll_inbox` | `POST /v1/agents/{id}/inbox/poll` | 同上 |
| `gofer_post_message` | `POST /v1/messages` | 同上 |
| `gofer_list_presence` | `GET /v1/agents/presence` | 同上 |
| `gofer_list_pending_interactions` | `GET /v1/interactions?status=pending` | 监督 driver 用；client 转发 |

> 现有 10 个 `gofer_*` 不变；新增 5 个挂同一 Backend 接口。view 投影（snake_case）留 mcpserver，两后端共用（E28 模式）。
