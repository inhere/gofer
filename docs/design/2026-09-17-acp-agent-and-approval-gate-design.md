...
| job 事件 | 只留生命周期：审批门的 `job.permission_requested|answered|timed_out`，加上回合结束时**一条** `job.acp_summary {tool_calls,thoughts,permissions,permissions_auto,stop_reason}`（`tool_calls` = 本回合的不同 toolCallId 数，`thoughts` = 收到的分片数，`permissions` = 本回合裁决的求批数）。在 worker 上跑时 `job.acp_summary` 与 permission 事件一样镜像回 host（`job.mirroredEventTypes`） |
| ↳ F4 修订（2026-09-22） | `job.permission_answered` 只表示**人工**作答：自动裁决（`approval: off` / `auto_allow_kind` / remembered / timeout 自动选项）不再各发一条（每次工具调用一条会把 `approval: off` 的 job 时间线刷满；同一决定已在 `acp.jsonl` 的 `auto:true` 记录与 stderr 紧凑事件里）。自动条数改由 `job.acp_summary` 的 `permissions_auto` 承担；超时自动作答仍发 `job.permission_timed_out`，其 detail 带上被选中的 `option_id`/`option_kind`。另见 `2026-09-22-windows-desktop-session-service-design.md` 的「实施中的补充决策」④ |
