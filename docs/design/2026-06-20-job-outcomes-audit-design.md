...
> 合并 [`../2026-06-20-enhancements-roadmap.md`](../2026-06-20-enhancements-roadmap.md) 的 **E1 产物回取 + E6 结构化结果 + E12 改了什么(diff) + E15 渲染命令**（强内聚，共用同一捕获钩子/schema/详情面）。E13 事件时间线机制不同，作相邻项不并入。bd epic `example-project-dhk`。
