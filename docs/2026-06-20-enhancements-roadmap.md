...
| E38 | **节点 / CLI 易用性补强** | 中 | 低 | 三个小补强：① **`gofer p ls`（project list）在 worker 节点无效**——现读**本地 config**（worker 侧无 projects 段）→ 应在有 server env 时**查 server `/v1/projects`**（或 `--remote` 开关），与 `job`/`wf` 一致。② **`GOFER_RUN_MODE=worker\|server\|mcp`**：据角色**自动定位默认 yaml** 加载（worker→`worker.yaml` / server→`config.yaml` / mcp→共用），免每次显式 `--worker-config`/`-c`。③ **✅ `gofer wf` 读 server env 已修**（commit `5be26be`，已部署容器 CLI 验证）：6 个 wf 子命令统一复用 `bindServerFlags`（绑入共享 `jobConnOpts`、带 `${GOFER_SERVER_ADDR/TOKEN}` 默认），删除各自空默认的 `--server/--token`，与 `job` 一致（零配置提交）。①② 仍待做。跨①轴。 |
