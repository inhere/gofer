...
| E38 | **节点 / CLI 易用性补强** | 中 | 低 | 三个小补强：① **`gofer p ls`（project list）在 worker 节点无效**——现读**本地 config**（worker 侧无 projects 段）→ 应在有 server env 时**查 server `/v1/projects`**（或 `--remote` 开关），与 `job`/`wf` 一致。② **`GOFER_RUN_MODE=worker\|server\|mcp`**：据角色**自动定位默认 yaml** 加载（worker→`worker.yaml` / server→`config.yaml` / mcp→共用），免每次显式 `--worker-config`/`-c`。③ **`gofer wf` 读 server env**：wf 各子命令 `--server/--token` 默认值**现为空、未取 `${GOFER_SERVER_ADDR/TOKEN}`**（`job` 经 `bindServerFlags` 已取）→ 复用 `bindServerFlags`/补 env 默认，与 `job` 一致（零配置提交）。**③ 可立即修**（一行级）。跨①轴。 |
