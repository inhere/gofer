// Package allocx provides overflow-safe allocation size hints.
//
// Every make() in gofer that pre-sizes a slice or map from element counts goes
// through Sum/Mul instead of spelling the arithmetic out inline. The reason is
// correctness, not style: `make([]T, 0, len(a)+len(b))` (or `len(m)*2`) computes
// the capacity hint with plain int arithmetic, and that computation is not
// guarded by anything — a wrapped result turns a purely cosmetic capacity hint
// into a panic ("makeslice: cap out of range") or a wild allocation. CodeQL
...
// far below any int limit so that the arithmetic in Sum/Mul can never wrap, and
// high enough (1M elements) that no realistic collection in gofer is clamped:
// these hints size env blocks, argv blocks and log lines, not bulk payloads.
const MaxHint = 1 << 20

// Sum returns a capacity hint for a collection that will hold the given element
// counts added together: Sum(len(a), len(b)) for "a followed by b". Non-positive
// parts contribute nothing (a nil map/slice has no elements). The result is
// always in [0, MaxHint], so the hint can never overflow and can never be
// rejected by make.
...
// Mul returns a capacity hint for a collection holding n groups of k elements
// each: Mul(len(m), 2) for "two entries per map key". Non-positive factors yield
// 0. The result is always in [0, MaxHint].
func Mul(n, k int) int {
	if n <= 0 || k <= 0 {
		return 0
	}
