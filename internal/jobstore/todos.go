...
func scanTodo(sc rowScanner) (PlanTodo, error) {
	...
	if t.Verify, err = decodeTodoVerify(verif); err != nil {
		return PlanTodo{}, err
	}
	if t.Status == "" {
...
func decodeTodoVerify(raw string) ([]string, error) {
	if strings.TrimSpace(raw) == "" {
		return nil, nil
	}
	var out []string
	if err := json.Unmarshal([]byte(raw), &out); err != nil {
		return nil, fmt.Errorf("verify_json: %w", err)
	}
	if len(out) == 0 {
		return nil, nil
	}
...
func (s *Store) InsertTodo(t PlanTodo) error {
  ...
  (todo_id, plan_id, job_id, title, done, status, started_at, done_at, note, sort,
   ...
   timeout_sec, dispatch_error, created_at, updated_at)
  VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
	s.writeMu.Lock()
