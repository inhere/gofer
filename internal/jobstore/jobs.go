...
type JobRecord struct {
	...
	FallbackJSON   string
	// UsageJSON is the agent's token/cost accounting for the job (SUP-01 E) as the
	// marshalled job.Usage, or "" when nothing was captured. Persisted so a finished
	// job still answers "what did this run cost?" after its logs rotated, and so
	// /v1/stats can aggregate it with SQL json_extract. Old rows COALESCE to "".
	UsageJSON string
...
const selectCols = `SELECT id, project_key, agent, runner, COALESCE(interactive,0), COALESCE(worker_id,''),
  ...
  COALESCE(requested_agent,''), COALESCE(fallback_json,''), COALESCE(usage_json,'') FROM jobs`
...
func (s *Store) UpsertJob(rec JobRecord) error {
  ...
  (id, project_key, agent, runner, interactive, worker_id, worker_instance_id, status, exit_code, cwd, result_dir,
   request_json, error, started_at, ended_at, updated_at, caller_id, request_id,
	    ...
	    failure_class, fell_back_from, fell_back_to, requested_agent, fallback_json, usage_json)
  VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
...
func (s *Store) CountActiveJobsByRole(role string) (int, error) {
	...
	args := make([]any, 0, util.CapSum(len(activeJobStatuses), 1))
...
func (s *Store) CountActiveJobsByCaller(callerID string, since int64) (int, error) {
	...
	args := make([]any, 0, util.CapSum(len(supervisedJobStatuses), 2))
...
func (s *Store) ReconcileOrphanJobs(ts int64, reason string, workerRunners []string) (int, error) {
  ...
  WHERE status IN (` + placeholders + `)`
	args := make([]any, 0, util.CapSum(len(nonTerminalJobStatuses), 3))
