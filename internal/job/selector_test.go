...
func TestSelectWorkerLabelsAllRequired(t *testing.T) {
	...
	if got := selectWorker(cands, []string{"gpu"}); got != "w-gpu" {
		t.Fatalf("single-label match = %q, want w-gpu", got)
	}
	if got := selectWorker(cands, []string{"gpu", "linux"}); got != "w-gpu" {
	...
	if got := selectWorker(cands, []string{"gpu", "windows"}); got != "" {
...
func TestSelectWorkerStaleExcluded(t *testing.T) {
	...
	if got := selectWorker(cands, []string{"gpu"}); got != "w-fresh" {
	...
	if got := selectWorker(only, []string{"gpu"}); got != "" {
...
func TestSelectWorkerOrdersByLoadThenAge(t *testing.T) {
	...
	if got := selectWorker(cands, []string{"gpu"}); got != "w-idle" {
	...
	if got := selectWorker(tie, []string{"gpu"}); got != "w-new" {
...
func TestSelectWorkerNoCandidates(t *testing.T) {
	if got := selectWorker(nil, []string{"gpu"}); got != "" {
	...
	if got := selectWorker(cands, []string{"gpu"}); got != "" {
