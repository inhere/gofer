...
func wfStep(name string, onFailure string, sub ...StepSpec) StepSpec {
	return StepSpec{
		...
		OnFailure:   onFailure,
		SubWorkflow: &WorkflowSpec{Title: name, Steps: sub},
	}
}
...
func waitChildWorkflow(t *testing.T, s *Service, parentID string, step int) jobstore.Workflow {
	...
	for time.Now().Before(deadline) {
		child, ok, err := s.meta.FindChildWorkflow(parentID, step)
		if err != nil {
...
func TestSubWorkflowNestedRunsThroughParent(t *testing.T) {
	...
	child, ok, err := s.meta.FindChildWorkflow(wf.ID, 2)
	if err != nil || !ok {
	...
	childJobs, _ := s.meta.ListWorkflowJobs(child.ID)
	if len(childJobs) != 2 {
		t.Fatalf("child ran %d step jobs, want 2", len(childJobs))
	}
	parentJobs, _ := s.meta.ListWorkflowJobs(wf.ID)
...
func TestSubWorkflowFailParentFailFast(t *testing.T) {
	...
	child := waitChildWorkflow(t, s, wf.ID, 2)
...
func TestSubWorkflowSweeperRecoversParentAdvance(t *testing.T) {
	...
	sub := WorkflowSpec{Steps: []StepSpec{echoStep("c1")}}
	parentSpec := WorkflowSpec{Steps: []StepSpec{
	...
	pj, _ := s.meta.ListWorkflowJobs(parentID)
	if len(pj) != 0 {
	...
	s.AdvanceRunningWorkflows(context.Background())

	final := waitWorkflow(t, s, parentID)
	if final.Status != jobstore.WorkflowDone {
...
// reaches terminal, MANY concurrent advanceWorkflow calls on the PARENT (the lost-and-
...
func TestSubWorkflowChildSubmitIdempotent(t *testing.T) {
	...
	sub := WorkflowSpec{Steps: []StepSpec{echoStep("c1")}}
	c1, err := s.SubmitWorkflowChild(sub, "alice", "wf-idem-parent", 1, 1)
	if err != nil {
		t.Fatalf("first SubmitWorkflowChild: %v", err)
	}
	c2, err := s.SubmitWorkflowChild(sub, "alice", "wf-idem-parent", 1, 1)
	if err != nil {
	...
	waitChildWorkflow(t, s, "wf-idem-parent", 1)
}
