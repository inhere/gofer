...
func retryFailStep(name string, max int, backoff []int) StepSpec {
	return StepSpec{
		...
		Retry:     &RetryPolicy{MaxAttempts: max, BackoffSec: backoff},
	}
}
...
func flakyStep(name, dir string, max int) StepSpec {
	...
	return StepSpec{
		...
		OnFailure: onFailureRetry,
		Retry:     &RetryPolicy{MaxAttempts: max, BackoffSec: []int{0}}, // immediate retry
	}
}
...
func TestOnFailureContinue(t *testing.T) {
	...
	if step3 := stepJobAttempt(jobs, 3, 1); step3 == nil || step3.Status != StatusDone {
		t.Fatalf("step3 should have run and be done after skip, got %+v", step3)
	}
	// A skipped event is recorded.
	if !hasWorkflowEvent(t, s, wf.ID, EventStepSkipped) {
...
func TestBackoffForPolicyIncreasesByAttempt(t *testing.T) {
	p := &RetryPolicy{MaxAttempts: 5, BackoffSec: []int{5, 50, 500}}
	cases := []struct {
		attempt int
		want    int
	}{
...
func TestRetryScheduleNextStepAt(t *testing.T) {
	...
	waitStepJobTerminal(t, s, wf.ID, 1) // attempt 1 fails
	got := waitStepAttempt(t, s, wf.ID, 2)
...
// after a retryable step fails, MANY concurrent advanceWorkflow calls (simulating
...
func TestRetryConcurrentSingleAttemptJob(t *testing.T) {
	...
	waitStepAttempt(t, s, wf.ID, 2)

	// HAMMER advanceWorkflow with the clock still BEFORE the backoff is due: the
	...
	got, _, _ := s.meta.GetWorkflow(wf.ID)
	if got.StepAttempt != 2 {
	...
	clk.set(nowUnix + 31)
	hammer(s, wf.ID, 32)
	a2 := waitStepAttemptJob(t, s, wf.ID, 1, 2) // attempt-2 job started exactly once
	...
	for i := 0; i < 5; i++ {
		...
		if wf, _, _ := s.meta.GetWorkflow(wf.ID); wf.Status != jobstore.WorkflowRunning {
			break
		}
...
func waitStepAttemptJob(t *testing.T, s *Service, wfID string, step, attempt int) *jobstore.JobRecord {
	t.Helper()
...
// hammer fires n concurrent advanceWorkflow calls and waits for all to return —
// the finish-hook + sweeper + duplicate storm simulation.
func hammer(s *Service, wfID string, n int) {
	done := make(chan struct{}, n)
	for i := 0; i < n; i++ {
		go func() { defer func() { done <- struct{}{} }(); s.advanceWorkflow(wfID) }()
	}
	for i := 0; i < n; i++ {
		<-done
	}
...
func TestWorkflowEventsTimeline(t *testing.T) {
	...
	events, err := s.ListWorkflowEvents(wf.ID, 0)
	if err != nil {
	...
	if events[0].Type != EventWorkflowSubmitted {
		t.Fatalf("first event = %s, want %s", events[0].Type, EventWorkflowSubmitted)
	}
	if last := events[len(events)-1]; last.Type != EventWorkflowTerminal {
		t.Fatalf("last event = %s, want %s", last.Type, EventWorkflowTerminal)
	}
	...
	if !containsInOrder(types, []string{
		EventWorkflowSubmitted, EventStepStarted, EventStepRetry, EventWorkflowTerminal,
	}) {
...
func waitStepAttempt(t *testing.T, s *Service, wfID string, want int) jobstore.Workflow {
	t.Helper()
	deadline := time.Now().Add(15 * time.Second)
	for time.Now().Before(deadline) {
...
func hasWorkflowEvent(t *testing.T, s *Service, wfID, eventType string) bool {
	t.Helper()
	events, err := s.ListWorkflowEvents(wfID, 0)
	if err != nil {
		t.Fatalf("ListWorkflowEvents: %v", err)
	}
