...
- **结果回传**：结果与日志写到 `<host_path>/tmp/gofer/<job_id>/`，容器与主机共享同一项目盘即可直接读回。
- **CLI = HTTP 封装**：`gofer job ...` 子命令直接调用 server 的 `/v1/jobs`，无需手写 curl。
...
gofer job run -p workspace -a exec --cwd . --wait -- go version
...
server:
  ...
  addr: 0.0.0.0:8765
  token_env: GOFER_TOKEN   # 优先从该 env 读取 bearer token（推荐）
  # token: ""                     # 也可内联（不要把真实 token 提交进仓库）
  allow_empty_token: false        # 必须显式 true 才能无 token 启动

storage:
  default_exchange_subdir: tmp              # 项目内数据交换子目录（默认 tmp）
  default_result_subdir: gofer   # 交换目录下的结果子目录
  # root: /var/lib/gofer   # 可选：全局集中 store（仅影响日志结果目录）
  # db_path: ""                       # 可选：SQLite 元数据库路径；空则
                                      #   显式 db_path > <root>/gofer.db > <config-dir>/gofer.db
...
- **配置目录**：默认 `~/.config/gofer/`，可用环境变量 `GOFER_CFG_DIR` 整体改到别处（同时影响 `config.yaml` 与全局 `.env` 的位置）。
- **`.env` 自动加载**：启动时（**先于读取任何配置**）按顺序加载两个 dotenv 文件，便于本地开发集中放置 `GOFER_TOKEN` 等敏感值（基于 `goutil/envutil`）：
  1. `<config-dir>/.env`（全局）
...
- **storage**：项目内交换 / 结果子目录的默认值；可选 `root` 把所有项目结果集中到一处（设了 `root` 后容器侧只能经 HTTP 回读，不再走共享盘）。job 元数据 / 索引 / 交互存于全局 SQLite 库（`db_path`，默认配置目录下 `gofer.db`），日志仍是结果目录文件；内存只保留运行中的 job，终态 job 落库后驱逐。可选 `retention` 周期清理旧终态 job 及其日志。
...
- 默认（未设 `storage.root`）：`<host_path>/<exchange_subdir>/<result_subdir>/<job_id>/`，即默认 `<host_path>/tmp/gofer/<job_id>/`。
...
`gofer <command> [sub] [--options] [args]`。所有命令都支持 `-c, --config` 指定配置文件。

```bash
# serve：启动 HTTP 控制面
gofer serve --addr 0.0.0.0:8765 --token dev-token   # 也支持 --allow-empty-token

# project：项目注册表
gofer project list
gofer project show workspace
gofer project add workspace --host-path /abs/path --allow-agent exec --allow-exec
gofer project remove workspace
gofer project validate workspace

# agent：agent 注册表
gofer agent list
gofer agent detect                # 跑各 agent 的 detect 探针，未装报 unavailable（不退非零）
gofer agent show codex

# job：通过 server 的 /v1/jobs 提交与管理（CLI 是 HTTP 封装）
gofer job run -p workspace -a exec --cwd . --wait -- go version
gofer job run -p workspace -a codex --prompt "总结本目录的测试失败用例"
gofer job show <id>
gofer job logs <id> --stream stdout      # stream: stdout|stderr
gofer job cancel <id>
```
...
`gofer mcp` 以 **stdio MCP server** 暴露同一套控制面能力（复用 `serve` 的 `job.Service` / project / agent 注册表，不重复执行逻辑）。stdout 为 MCP 协议通道，命令启动后不向 stdout 输出任何日志。

```bash
# 启动（由 MCP 客户端拉起，通过 stdin/stdout 通信）
gofer mcp --config /abs/path/to/config.yaml
```

给 Claude Code / Codex 等客户端的 MCP 配置示例（`command` 用 `gofer` 二进制的绝对路径）：

```json
{
  "mcpServers": {
    "gofer": {
      "command": "/abs/path/to/gofer",
...
# 2. 起服务（Web 默认开启）
gofer serve --addr 0.0.0.0:8765 --token dev-token
...
│ curl host.docker.internal:8765/v1/jobs │ ──▶ │ gofer serve (本程序)  │
│   Authorization: Bearer <token>     │        │   在项目真实 cwd 异步执行     │
│ 读 ./tmp/gofer/<id>/...  │ ◀──    │   结果写 <host_path>/tmp/... │
└─────────────────────────────────────┘ 共享盘 └──────────────────────────────┘
```
...
```bash
gofer job run -p workspace -a codex    --prompt "在本目录跑测试并总结失败用例"
gofer job run -p workspace -a claude   --prompt "审查本目录的改动并给出风险点"
gofer job run -p workspace -a opencode --prompt "解释本目录的整体结构"
```
...
> 当前主机上**仍在运行旧 `codex-bridge`**（用 `X-Bridge-Token` 头、旧 `/run` + `/result/{id}` 接口）。**在主机 bridge 切到新 `gofer serve` 二进制并验证之前，请勿改动根 `CLAUDE.md`，也不要删除 `tools/codex-bridge`**——否则文档会与仍存活的服务契约不符。下面是届时由人工在主机侧执行的精确清单。

### A. 重启主机 bridge 为新二进制

```bash
# 主机侧
cd /d/work/inhere/hyy-ai-inspect/tools/gofer
make build                       # 产出 dist/gofer（或 go build -o dist/gofer ./cmd/gofer）

# 登记工作空间项目（一次性，若尚未登记）
./dist/gofer project add workspace \
  --host-path /d/work/inhere/hyy-ai-inspect \
  --container-path /workspace \
  --default-agent codex \
  --allow-agent codex --allow-agent claude --allow-agent exec \
...
export GOFER_TOKEN=tok-for-docker-dev
./dist/gofer serve --addr 0.0.0.0:8765
```
...
- 新 CLI `gofer`：`serve` / `project` / `agent` / `job`（`mcp` 占位）。
...
- 结果目录改为 `tmp/gofer/<job_id>`（可选全局 `storage.root`）。
