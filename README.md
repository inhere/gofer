...
- **Agent output**: a structured agent (`omp --mode json`, `claude --output-format stream-json`) writes one JSON event per line; with `output_format: ndjson` gofer filters the per-token incremental events out **at capture time**, so `stdout.log` keeps only the readable events (the `session` row is always kept) — 10-30x smaller, and the web job detail can render it as a timeline (`ndjson_keep` tunes the whitelist, `ndjson_raw` also keeps the unfiltered `stdout.raw.log`; the kept/dropped counts land in `ndjson_kept` / `ndjson_dropped`).
